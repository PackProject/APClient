...
int GlfClubUp(int currentClubIdx) {
    for (int i = currentClubIdx - 1; i >= 0; i--) {
        if (Cmn::ItemMgr::GetInstance().IsGlfClubUnlock(i)) {
            return i;
        }
    }
...
int GetMaxClubID() {
    ...
    for (int i = 0; i < Cmn::ItemMgr::GLF_CLUB_COUNT - 1; i++) {
        if (Cmn::ItemMgr::GetInstance().IsGlfClubUnlock(i)) {
            maxClubID = i;
        }
    }
...
void GlfSetBlindFlag(bool hold2) {
    if (hold2 || !Cmn::ItemMgr::GetInstance().IsGlfHUD()) {
...
bool GlfDisableTurn() {
    return Cmn::ItemMgr::GetInstance().IsGlfTurn();
}
...
bool GlfHasBackSpin() {
    return Cmn::ItemMgr::GetInstance().IsGlfSpin();
}
