...
#define AVDT_PSC                                                               \
    (AVDT_PSC_TRANS | AVDT_PSC_MUX | AVDT_PSC_REPORT | AVDT_PSC_DELAY_RPT)
...
typedef struct {
    ...
    BOOLEAN proc_busy;  /* TRUE when a discover or get capabilities procedure in
                           progress */
    UINT8 proc_param;   /* Procedure parameter; either SEID for get capabilities
    ...
    BOOLEAN reconn;  /* If TRUE, reinitiate connection after transitioning from
...
typedef struct {
    ...
    UINT32 frag_org_len; /* original length before fragmentation of receiving
                            media packet */
    UINT8* p_next_frag;  /* next fragment to send */
    UINT8*
        p_media_buf; /* buffer for media packet assigned by AVDT_SetMediaBuf */
    UINT32 media_buf_len; /* length of buffer for media packet assigned by
                             AVDT_SetMediaBuf */
#endif
} tAVDT_SCB;
...
extern void avdt_ccb_event(tAVDT_CCB* p_ccb, UINT8 event,
...
extern UINT8 avdt_scb_verify(tAVDT_CCB* p_ccb, UINT8 state, UINT8* p_seid,
                             UINT16 num_seid, UINT8* p_err_code);
...
extern void avdt_scb_queue_frags(tAVDT_SCB* p_scb, UINT8** pp_data,
                                 UINT32* p_data_len, BUFFER_Q* pq);

/* msg function declarations */
extern BOOLEAN avdt_msg_send(tAVDT_CCB* p_ccb, BT_HDR* p_msg);
extern void avdt_msg_send_cmd(tAVDT_CCB* p_ccb, void* p_scb, UINT8 sig_id,
                              tAVDT_MSG* p_params);
extern void avdt_msg_send_rsp(tAVDT_CCB* p_ccb, UINT8 sig_id,
                              tAVDT_MSG* p_params);
extern void avdt_msg_send_rej(tAVDT_CCB* p_ccb, UINT8 sig_id,
                              tAVDT_MSG* p_params);
extern void avdt_msg_send_grej(tAVDT_CCB* p_ccb, UINT8 sig_id,
                               tAVDT_MSG* p_params);
...
extern tAVDT_TC_TBL* avdt_ad_tc_tbl_by_st(UINT8 type, tAVDT_CCB* p_ccb,
                                          UINT8 state);
...
extern tAVDT_TC_TBL* avdt_ad_tc_tbl_by_type(UINT8 type, tAVDT_CCB* p_ccb,
                                            tAVDT_SCB* p_scb);
extern UINT8 avdt_ad_write_req(UINT8 type, tAVDT_CCB* p_ccb, tAVDT_SCB* p_scb,
                               BT_HDR* p_buf);
extern void avdt_ad_open_req(UINT8 type, tAVDT_CCB* p_ccb, tAVDT_SCB* p_scb,
                             UINT8 role);
