...
#define AVDT_RECOV_MNMP_MIN                                                    \
    0x01 /* min value for maximum number of media packets */
#define AVDT_RECOV_MNMP_MAX                                                    \
    0x18 /* max value for maximum number of media packets */
...
#define AVDT_ALH_LCODE_NONE                                                    \
    0x00 /* No length field present. Take length from l2cap */
#define AVDT_ALH_LCODE_16BIT 0x01 /* 16bit length field */
#define AVDT_ALH_LCODE_9BITM0                                                  \
    0x02 /* 9 bit length field, MSB = 0, 8 LSBs in 1 octet following */
#define AVDT_ALH_LCODE_9BITM1                                                  \
    0x03 /* 9 bit length field, MSB = 1, 8 LSBs in 1 octet following */
...
#define AVDT_MSG_PRS_SIG(p, sig) sig = *(p)++ & 0x3F;

#define AVDT_MSG_PRS_SEID(p, seid) seid = *(p)++ >> 2;

#define AVDT_MSG_PRS_PKT_TYPE(p, pkt) pkt = (*(p) >> 2) & 0x03;
...
#define AVDT_MSG_BLD_SIG(p, sig) *(p)++ = (UINT8)(sig);

#define AVDT_MSG_BLD_SEID(p, seid) *(p)++ = (UINT8)((seid) << 2);

#define AVDT_MSG_BLD_ERR(p, err) *(p)++ = (UINT8)(err);

#define AVDT_MSG_BLD_PARAM(p, param) *(p)++ = (UINT8)(param);

#define AVDT_MSG_BLD_NOSP(p, nosp) *(p)++ = (UINT8)(nosp);
