...
#define GATT_CREATE_CONN_ID(tcb_idx, gatt_if)                                  \
    ((UINT16)((((UINT8)(tcb_idx)) << 8) | ((UINT8)(gatt_if))))
...
typedef struct {
    ...
    UINT8 value[GATT_MAX_MTU_SIZE]; /* pointer to the attribute value to be
                                       found */
...
typedef struct {
    ...
    TIMER_LIST_ENT
    ind_ack_timer_ent; /* local app confirm to indication timer */
...
/* logic channel */
typedef struct {
    UINT16 next_disc_start_hdl; /* starting handle for the next inc srvv
                                   discovery */
...
extern tGATT_STATUS attp_send_cl_msg(tGATT_TCB* p_tcb, UINT16 clcb_idx,
                                     UINT8 op_code, tGATT_CL_MSG* p_msg);
...
extern UINT32 gatt_add_sdp_record(tBT_UUID* p_uuid, UINT16 start_hdl,
                                  UINT16 end_hdl);
extern BOOLEAN gatt_parse_uuid_from_cmd(tBT_UUID* p_uuid, UINT16 len,
                                        UINT8** p_data);
...
extern void gatt_sr_get_sec_info(BD_ADDR rem_bda, BOOLEAN le_conn,
...
extern tGATT_STATUS gatt_send_error_rsp(tGATT_TCB* p_tcb, UINT8 err_code,
                                        UINT8 op_code, UINT16 handle,
                                        BOOLEAN deq);
extern void gatt_dbg_display_uuid(tBT_UUID bt_uuid);

extern tGATTS_PENDING_NEW_SRV_START*
gatt_sr_is_new_srv_chg(tBT_UUID* p_app_uuid128, tBT_UUID* p_svc_uuid,
                       UINT16 svc_inst);
...
gatt_find_hdl_buffer_by_app_id(tBT_UUID* p_app_uuid128, tBT_UUID* p_svc_uuid,
                               UINT16 svc_inst);
...
extern BOOLEAN gatt_is_bg_dev_for_app(tGATT_BG_CONN_DEV* p_dev,
                                      tGATT_IF gatt_if);
...
extern UINT8 gatt_sr_find_i_rcb_by_app_id(tBT_UUID* p_app_uuid128,
                                          tBT_UUID* p_svc_uuid,
                                          UINT16 svc_inst);
...
extern UINT32 gatt_sr_enqueue_cmd(tGATT_TCB* p_tcb, UINT8 op_code,
                                  UINT16 handle);
...
extern void gatt_sr_update_cback_cnt(tGATT_TCB* p_tcb, tGATT_IF gatt_if,
                                     BOOLEAN is_inc, BOOLEAN is_reset_first);
...
extern UINT8 gatt_send_write_msg(tGATT_TCB* p_tcb, UINT16 clcb_idx,
                                 UINT8 op_code, UINT16 handle, UINT16 len,
...
extern BOOLEAN gatt_cmd_enq(tGATT_TCB* p_tcb, UINT16 clcb_idx, BOOLEAN to_send,
...
/* gatt_db.c */
extern BOOLEAN gatts_init_service_db(tGATT_SVC_DB* p_db, tBT_UUID service,
                                     BOOLEAN is_pri, UINT16 s_hdl,
                                     UINT16 num_handle);
...
extern tGATT_STATUS
gatts_db_read_attr_value_by_type(tGATT_TCB* p_tcb, tGATT_SVC_DB* p_db,
                                 UINT8 op_code, BT_HDR* p_rsp, UINT16 s_handle,
                                 UINT16 e_handle, tBT_UUID type, UINT16* p_len,
                                 tGATT_SEC_FLAG sec_flag, UINT8 key_size,
                                 UINT32 trans_id, UINT16* p_cur_handle);
extern tGATT_STATUS gatts_read_attr_value_by_handle(
    tGATT_TCB* p_tcb, tGATT_SVC_DB* p_db, UINT8 op_code, UINT16 handle,
    UINT16 offset, UINT8* p_value, UINT16* p_len, UINT16 mtu,
    tGATT_SEC_FLAG sec_flag, UINT8 key_size, UINT32 trans_id);
extern tGATT_STATUS
gatts_write_attr_perm_check(tGATT_SVC_DB* p_db, UINT8 op_code, UINT16 handle,
                            UINT16 offset, UINT8* p_data, UINT16 len,
                            tGATT_SEC_FLAG sec_flag, UINT8 key_size);
extern tGATT_STATUS gatts_read_attr_perm_check(tGATT_SVC_DB* p_db,
                                               BOOLEAN is_long, UINT16 handle,
                                               tGATT_SEC_FLAG sec_flag,
                                               UINT8 key_size);
extern void gatts_update_srv_list_elem(UINT8 i_sreg, UINT16 handle,
                                       BOOLEAN is_primary);
