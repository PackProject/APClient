...
typedef struct {
    ...
    UINT8 ch_state;   /* L2CAP channel state */
    UINT8 ch_flags;   /* L2CAP configuration flags */
    BT_HDR* p_tx_msg; /* Message to be sent - in case the browsing channel is
                         not open when MsgReg is called */
...
extern void avct_lcb_event(tAVCT_LCB* p_lcb, UINT8 event,
                           tAVCT_LCB_EVT* p_data);
#if (AVCT_BROWSE_INCLUDED == TRUE)
extern void avct_bcb_event(tAVCT_BCB* p_bcb, UINT8 event,
                           tAVCT_LCB_EVT* p_data);
...
extern void avct_ccb_dealloc(tAVCT_CCB* p_ccb, UINT8 event, UINT16 result,
                             BD_ADDR bd_addr);
