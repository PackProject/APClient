...
#define OBX_MAX_CONN_HDR_EXTRA                                                 \
    8 /* 5/conn id, 2/ssn, 2/srm 2/srm_param - (5/setpath + 5/conn_id -        \
         7/conn) */
...
#define OBX_SESSION_INFO_SIZE                                                  \
    32 /* OBX_SESSION_ID_SIZE + 4(local nonce) + 4 (connection id) + 4         \
          (timeout) + 2(mtu) + 1(state) + 1(srm)  */
#define OBX_SESSION_INFO_NONCE_IDX                                             \
    16 /* The index to the (local nonce) in session info */
#define OBX_SESSION_INFO_ID_IDX                                                \
    20 /* The index to the (connection id) in session info */
#define OBX_SESSION_INFO_TO_IDX                                                \
    24 /* The index to the (timeout) in session info */
#define OBX_SESSION_INFO_MTU_IDX                                               \
    28 /* The index to peer MTU in session info                                \
        */
#define OBX_SESSION_INFO_ST_IDX                                                \
    30 /* The index to sr/cl state in session info */
...
enum {
    ...
    OBX_PT_DELETE, /* Delete request - a Put request with NO Body or End-of-Body
                      header. */
    OBX_PT_CREATE  /* Create-Empty request - a Put request with an empty
                      End-of-Body header. */
};
typedef UINT8 tOBX_PUT_TYPE;

/* SetPath Request Flags - the following definitions can be ORed if both flags
 * are wanted */
#define OBX_SPF_BACKUP                                                         \
    0x01 /* Backup a level before applying name(equivalent to ../) */
#define OBX_SPF_NO_CREATE                                                      \
    0x02 /* Don't create directory if it does not exist, return an error       \
            instead. */
typedef UINT8 tOBX_SETPATH_FLAG;

/* Authentication Challenge Options */
#define OBX_AO_NONE                                                            \
    0x00 /* If this is used in OBX_StartServer and the authenticate            \
          * flag is TRUE, the optional Challenge Information (tag 0x01)        \
          * will not be sent. */
#define OBX_AO_USR_ID                                                          \
    0x01 /* Set this bit to make the client respond with the user ID. */
...
typedef struct {
    ...
    BOOLEAN
    final; /* TRUE, if this is the final packet of this PUT transaction. */
...
typedef struct {
    UINT8 ssn; /* session sequence number */
    BOOLEAN
    final; /* TRUE, if this is the final packet of this GET transaction. */
} tOBX_GET_EVT;

typedef struct {
    UINT8 ssn;          /* session sequence number */
    BD_ADDR peer_addr;  /* The peer Bluetooth Address. */
    UINT16 mtu;         /* The peer MTU. This element is associated with
                           OBX_CONNECT_REQ_EVT and OBX_CONNECT_RSP_EVT. */
    tOBX_HANDLE handle; /* the OBX handle returned by OBX_StartServer(),
                           OBX_CreateSession() and OBX_ConnectReq() */
    BOOLEAN no_rsp;     /* TRUE, when the event is generated as a part of RESUME
                           SESSION */
} tOBX_CONN_EVT;
...
typedef struct {
    ...
    BD_ADDR peer_addr;    /* The peer Bluetooth Address. */
    UINT8* p_sess_info;   /* The session ID and the local nonce for a reliable
                             session, a reference to the location in OBEX control
                             block or NULL */
    UINT32 timeout;    /* The number of seconds remaining in suspend. 0xffff if
                          infinite. */
...
typedef union {
    UINT8 ssn;          /* session sequence number */
    tOBX_CONN_EVT conn; /* This element is associated with OBX_CONNECT_REQ_EVT
                           and OBX_CONNECT_RSP_EVT. */
    tOBX_SESS_EVT sess; /* This element is associated with OBX_SESSION_RSP_EVT
                           and OBX_SESSION_REQ_EVT. */
    tOBX_PUT_EVT put;   /* This element is associated with OBX_PUT_REQ_EVT. */
    tOBX_SETPATH_EVT
        sp; /* This element is associated with OBX_SETPATH_REQ_EVT. */
    tOBX_ACT_EVT
        action;       /* This element is associated with OBX_ACTION_REQ_EVT */
    tOBX_GET_EVT get; /* This element is associated with OBX_GET_REQ_EVT. TRUE,
                         if this is the final packet that contains the OBEX
                         headers for this GET request. */
} tOBX_EVT_PARAM;
...
enum {
    ...
    OBX_SESSION_REQ_EVT, /* A Crease Session or Resume Session request is
                            received by the server. Call OBX_SessionRsp(). */
    OBX_DISCONNECT_REQ_EVT,
    OBX_PUT_REQ_EVT, /* could have param indicate delete or create? */
    OBX_GET_REQ_EVT,
    OBX_SETPATH_REQ_EVT, /* param = flags */
    OBX_ABORT_REQ_EVT,
    OBX_ACTION_REQ_EVT, /* An Action request is received by the server. Call
                           OBX_ActionRsp(). */

    /* client events */
    OBX_CONNECT_RSP_EVT, /* param = packet MTU */
    OBX_SESSION_RSP_EVT, /* A response for Create Session or Resume Session is
                            received by the client. The client needs to remember
                            the session id. The session id is to be used in
                            calling OBX_ResumeSession, if the current session is
    ...
    /* common events */
    OBX_SESSION_INFO_EVT, /* the session information event to resume the
                             session. */
    OBX_CLOSE_IND_EVT,    /* when transport goes down; p_pkt = NULL; no response
                             needed */
...
#define OBX_MAX_OFFSET_IND                                                     \
    OBX_ABORT_RSP_EVT /* This is used to access obx_hdr_start_offset */
...
#define OBX_INVALID_CONN_ID                                                    \
    0xFFFFFFFF /* invalid connection ID per IrOBEX spec */
#define OBX_INFINITE_TIMEOUT 0xFFFFFFFF

/* Header Identifier Data Type Constants */
#define OBX_HI_TYPE_MASK                                                       \
    0xC0 /* This mask get the encoding (data type) of the header ID. */
...
#define OBX_HV_SRM_PARAM_MORE                                                  \
    0x00 /* SRM Param header value - request additional packet */
#define OBX_HV_SRM_PARAM_WAIT                                                  \
    0x01 /* SRM Param header value - wait for next req/rsp */
#define OBX_HV_SRM_PARAM_COMBO                                                 \
    0x02 /* SRM Param header value - next and wait                             \
          */
...
#define OBX_MAX_SESS_PARAM_TRIP                                                \
    7 /* max number of TLV for session operations                              \
       */

#define OBX_LEN_SESS_PARAM_SESS_OP 1
#define OBX_LEN_SESS_PARAM_OBJ_OFF                                             \
    4 /* this value varies, so it needs to be verified on the receiving side   \
       */
...
typedef void(tOBX_SR_CBACK)(tOBX_HANDLE shandle, tOBX_EVENT event,
                            tOBX_EVT_PARAM param, BT_HDR* p_pkt);
/* Client Callback type: */
typedef void(tOBX_CL_CBACK)(tOBX_HANDLE handle, tOBX_EVENT event,
                            UINT8 rsp_code, tOBX_EVT_PARAM param,
                            BT_HDR* p_pkt);

typedef struct {
    UINT16 len;                       /* Length of target header. */
    UINT8 target[OBX_MAX_TARGET_LEN]; /* The byte sequence that describes the
                                         target header. */
} tOBX_TARGET;
...
typedef struct {
    ...
    UINT16 mtu;
    UINT8 scn; /* The RFCOMM SCN number that this server listens for incoming
                  requests. 0, if do not wish to listen to connection from
                  RFCOMM. */
    ...
    UINT8 realm_len;
    UINT8 max_sessions;
    BOOLEAN get_nonf; /* report GET non-final request event. If FALSE, GET
                         response is sent automatically */
    UINT16 psm;   /* The L2CAP PSM number that this server listens for incoming
                     requests. 0, if do not wish to listen to connection from
                     L2CAP. */
    UINT32 nonce; /* This is converted to UINT8[16] internally before adding to
                     the OBEX header. This value is copied to the server control
                     block and is increased after each use. 0, if only legacy
                     OBEX (unreliable) session is desired. */
    BOOLEAN srm;  /* TRUE, to support single response mode. */
    UINT8 max_suspend; /* Max number of suspended session. must be less than
                          OBX_MAX_SUSPEND_SESSIONS. ignored, if nonce is 0 */
} tOBX_StartParams;
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_StartServer(tOBX_StartParams* p_params,
                                           tOBX_HANDLE* p_handle);
...
**				suspended reliable session to the server control
*block
**
** Returns      OBX_SUCCESS, if successful.
**              OBX_BAD_HANDLE, if the handle is not valid.
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_AddSuspendedSession(tOBX_HANDLE shandle,
                                                   BD_ADDR peer_addr,
                                                   UINT8* p_sess_info,
                                                   UINT32 timeout, UINT8 ssn,
                                                   UINT32 offset);
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_ConnectRsp(tOBX_HANDLE shandle, UINT8 rsp_code,
                                          BT_HDR* p_pkt);
...
**
** Description  This function is called to respond to a request to create a
*reliable session.
**
**
** Returns      OBX_SUCCESS, if successful.
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_SetPathRsp(tOBX_HANDLE shandle, UINT8 rsp_code,
                                          BT_HDR* p_pkt);
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_PutRsp(tOBX_HANDLE shandle, UINT8 rsp_code,
                                      BT_HDR* p_pkt);
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_GetRsp(tOBX_HANDLE shandle, UINT8 rsp_code,
                                      BT_HDR* p_pkt);
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_AbortRsp(tOBX_HANDLE shandle, UINT8 rsp_code,
                                        BT_HDR* p_pkt);
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_ActionRsp(tOBX_HANDLE shandle, UINT8 rsp_code,
                                         BT_HDR* p_pkt);
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_DisconnectRsp(tOBX_HANDLE shandle,
                                             UINT8 rsp_code, BT_HDR* p_pkt);
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_Password(tOBX_HANDLE shandle, UINT8* p_password,
                                        UINT8 password_len, UINT8* p_userid,
                                        UINT8 userid_len);
...
**
** Description  This function is called to get the RFCOMM port handle for the
*obex connection.
**
**
** Returns      OBX_SUCCESS, if successful.
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_GetPortHandle(tOBX_HANDLE handle,
                                             UINT16* port_handle);
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_ConnectReq(BD_ADDR bd_addr, UINT8 scn,
                                          UINT16 mtu, tOBX_CL_CBACK* p_cback,
                                          tOBX_HANDLE* p_handle, BT_HDR* p_pkt);
...
**              If p_session_id is not NULL, it tries to find an suspended
*session
**                  with matching session_id.
**              If scn is not 0, it allocates a control block for this new
*session.
**              Otherwise, it allocates a control block for the given PSM.
**              The associated virtual PSM assigned by L2CAP is returned in
*p_psm
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_AllocSession(UINT8* p_session_info, UINT8 scn,
                                            UINT16* p_psm,
                                            tOBX_CL_CBACK* p_cback,
                                            tOBX_HANDLE* p_handle);
...
**
** Description  This function registers a client entity to OBEX.
**              It may send a CreateSession request and wait for CreateSession
*response.
**              It sends a CONNECT request to the server specified by the API
*parameters.
**
** Returns      OBX_SUCCESS, if successful.
**              OBX_NO_RESOURCES, if OBX does not resources
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_CreateSession(BD_ADDR bd_addr, UINT16 mtu,
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_ResumeSession(BD_ADDR bd_addr, UINT8 ssn,
                                             UINT32 offset, tOBX_HANDLE handle);
...
**              For Resume: this function registers a client entity to OBEX and
*resumes
**              a previously interrupted reliable session.
**
** Returns      OBX_SUCCESS, if successful.
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_SessionReq(tOBX_HANDLE handle,
                                          tOBX_SESS_OP opcode, UINT32 timeout);
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_SetPathReq(tOBX_HANDLE handle, UINT8 flags,
                                          BT_HDR* p_pkt);
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_PutReq(tOBX_HANDLE handle, BOOLEAN final,
                                      BT_HDR* p_pkt);
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_GetReq(tOBX_HANDLE handle, BOOLEAN final,
                                      BT_HDR* p_pkt);
...
**
** Description  This function sends a Disconnect request to the connected
*server.
**
** Returns      OBX_SUCCESS, if successful.
**              OBX_BAD_HANDLE, if the handle is not valid.
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS OBX_ActionReq(tOBX_HANDLE handle,
                                         tOBX_ACTION action_id, BT_HDR* p_pkt);
...
**
*******************************************************************************/
OBX_API extern tOBX_STATUS
OBX_AuthResponse(tOBX_HANDLE handle, UINT8* p_password, UINT8 password_len,
                 UINT8* p_userid, UINT8 userid_len, BOOLEAN authenticate);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_AddNameHdr(BT_HDR* p_pkt, UINT16* p_name,
                                      UINT16 len);
...
**
*******************************************************************************/
OBX_API extern UINT16 OBX_CharToWchar(UINT16* w_str, char* a_str,
                                      UINT16 w_size);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_AddDescrHdr(BT_HDR* p_pkt, UINT16* p_descr,
                                       UINT16 len);
...
**
**              NOTE: The target header must be the first header in an OBEX
*message.
**
** Returns      TRUE, if the header is added successfully.
**              FALSE, if the operation failed. p_pkt is not altered.
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_AddTargetHdr(BT_HDR* p_pkt, UINT8* p_target,
                                        UINT16 len);
...
**              to an OBEX packet.
**
**              NOTE: The body header must be the last header in an OBEX
*message.
**
** Returns      TRUE, if the header is added successfully.
**              FALSE, if the operation failed. p_pkt is not altered.
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_AddBodyHdr(BT_HDR* p_pkt, UINT8* p_body, UINT16 len,
                                      BOOLEAN end);
...
**              the actual value of the body has been copied into the OBEX
*packet.
**
** Returns      void
**
*******************************************************************************/
OBX_API extern void OBX_AddBodyEnd(BT_HDR* p_pkt, UINT8* p, UINT16 len,
                                   BOOLEAN end);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_AddAppParamHdr(BT_HDR* p_pkt,
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_AddDestNameHdr(BT_HDR* p_pkt, UINT16* p_dest,
                                          UINT16 len);
...
**
** Description  This function is called to add an OBEX Permission header to an
*OBEX
**              packet.
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_AddPermissionHdr(BT_HDR* p_pkt, UINT8 user,
                                            UINT8 group, UINT8 other);
...
**
** Description  This function is called to add a header with type as byte
*sequence
**              to an OBEX packet.
**
** Returns      TRUE, if the header is added successfully.
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_AddByteStrHdr(BT_HDR* p_pkt, UINT8 id, UINT8* p_data,
                                         UINT16 len);
...
**
** Description  This function is called to add a header with type as Unicode
*string
**              to an OBEX packet.
**
** Returns      TRUE, if the header is added successfully.
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_AddUnicodeHdr(BT_HDR* p_pkt, UINT8 id,
                                         UINT16* p_data, UINT16 len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_AddTriplet(BT_HDR* p_pkt, UINT8 id,
                                      tOBX_TRIPLET* p_triplet, UINT8 num);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadNameHdr(BT_HDR* p_pkt, UINT16* p_name,
                                       UINT16* p_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadAsciiNameHdr(BT_HDR* p_pkt, char* p_name,
                                            UINT16 max_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadTypeHdr(BT_HDR* p_pkt, UINT8** p_type,
                                       UINT16* p_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadTimeHdr(BT_HDR* p_pkt, UINT8** p_time,
                                       UINT16* p_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadDescrHdr(BT_HDR* p_pkt, UINT16* p_descr,
                                        UINT16* p_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadDestNameHdr(BT_HDR* p_pkt, UINT16* p_dest,
                                           UINT16* p_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadAsciiDescrHdr(BT_HDR* p_pkt, char* p_descr,
                                             UINT16 max_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadAsciiDestNameHdr(BT_HDR* p_pkt, char* p_dest,
                                                UINT16 max_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadTargetHdr(BT_HDR* p_pkt, UINT8** p_target,
                                         UINT16* p_len, UINT8 next);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadHttpHdr(BT_HDR* p_pkt, UINT8** p_http,
                                       UINT16* p_len, UINT8 next);
...
**
*******************************************************************************/
OBX_API extern UINT8 OBX_ReadBodyHdr(BT_HDR* p_pkt, UINT8** p_body,
                                     UINT16* p_len, BOOLEAN* p_end);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadWhoHdr(BT_HDR* p_pkt, UINT8** p_who,
                                      UINT16* p_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadAppParamHdr(BT_HDR* p_pkt, UINT8* p_tag,
                                           UINT8** p_app_param, UINT8* p_len,
                                           UINT8 next);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadPermissionHdr(BT_HDR* p_pkt, UINT8* p_user,
                                             UINT8* p_group, UINT8* p_other);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_Read4ByteHdr(BT_HDR* p_pkt, UINT8 id,
                                        UINT32* p_data);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadByteStrHdr(BT_HDR* p_pkt, UINT8 id,
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadUnicodeHdr(BT_HDR* p_pkt, UINT8 id,
                                          UINT16* p_data, UINT16* p_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadTriplet(BT_HDR* p_pkt, UINT8 id,
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadAuthParams(BT_HDR* p_pkt, UINT8** p_userid,
                                          UINT8* p_len, BOOLEAN* is_challenged,
                                          tOBX_AUTH_OPT* p_option);
...
**
*******************************************************************************/
OBX_API extern UINT16 utfc_16_to_8(UINT8* p_utf8, UINT16 utf8_len,
                                   UINT16* p_utf16, UINT16 utf16_len);
...
**
*******************************************************************************/
OBX_API extern UINT16 utfc_8_to_16(UINT16* p_utf16, UINT16 utf16_len,
                                   UINT8* p_utf8);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadUtf8NameHdr(BT_HDR* p_pkt, UINT8* p_name,
                                           UINT16 max_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadUtf8DescrHdr(BT_HDR* p_pkt, UINT8* p_descr,
                                            UINT16 max_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_ReadUtf8DestNameHdr(BT_HDR* p_pkt, UINT8* p_dest,
                                               UINT16 max_len);
...
**
*******************************************************************************/
OBX_API extern BOOLEAN OBX_VerifyResponse(UINT32 nonce_u32, UINT8* p_password,
                                          UINT8 password_len,
                                          UINT8* p_response);
