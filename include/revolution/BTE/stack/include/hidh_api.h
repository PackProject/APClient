...
tHID_HOST_DEV_CALLBACK HID_HDEV_EVT_OPEN   Connected to device with Interrupt
and Control Channels in OPEN state. Data = NA HID_HDEV_EVT_CLOSE  Connection
with device is closed.   Data=reason code. HID_HDEV_EVT_RETRYING   Lost
connection is being re-connected. Data=Retrial number HID_HDEV_EVT_IN_REPORT
Device sent an input report     Data=Report Type pdata= pointer to BT_HDR (GKI
buffer having report data.) HID_HDEV_EVT_HANDSHAKE  Device sent SET_REPORT
Data=Result-code pdata=NA. HID_HDEV_EVT_VC_UNPLUG  Device sent Virtual Unplug
Data=NA. pdata=NA.
*/
...
typedef void(tHID_HOST_DEV_CALLBACK)(
    UINT8 dev_handle, UINT8 event, /* Event from HID-DEVICE. */
...
**
*******************************************************************************/
HID_API extern tHID_STATUS
HID_HostGetSDPRecord(BD_ADDR addr, tSDP_DISCOVERY_DB* p_db, UINT32 db_len,
...
**
** Description      This removes the device from list devices that host has to
*manage.
**
** Returns          tHID_STATUS
**
...
** Returns         tHID_STATUS
*******************************************************************************/
HID_API extern tHID_STATUS HID_HostSetSecurityLevel(char serv_name[],
                                                    UINT8 sec_lvl);
