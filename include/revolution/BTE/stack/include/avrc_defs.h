...
#define AVRC_PACKET_LEN                                                        \
    512 /* Per the spec, you must support 512 byte RC packets */

#define AVRC_MIN_CONTROL_MTU                                                   \
    48 /* Per the spec, minimum MTU for the control channel */
#define AVRC_MIN_BROWSE_MTU                                                    \
    335 /* Per the spec, minimum MTU for the browsing channel */
...
#define AVRC_CMD_NOTIF                                                         \
    3 /* Used for receiving notification of a change in a device’s state */
...
/* opcodes 80-9F and E0-FF are not used by 1394ta.Sneak one for the browsing
 * channel */
...
/* the only pass through vendor unique commands defined by AVRC is the group
 * navigation commands The len for vendor unique data is 5 */
#define AVRC_PASS_THRU_GROUP_LEN 5

#define AVRC_PDU_INVALID 0xff
/* 6.15.3 error status code for general reject */
#define AVRC_STS_BAD_CMD                                                       \
    0x00 /* Invalid command, sent if TG received a PDU that it did not         \
            understand. */
#define AVRC_STS_BAD_PARAM                                                     \
    0x01 /* Invalid parameter, sent if the TG received a PDU with a parameter  \
            ID that it did not understand. Sent if there is only one parameter \
            ID in the PDU. */
#define AVRC_STS_NOT_FOUND                                                     \
    0x02 /* Specified parameter not found., sent if the parameter ID is        \
            understood, but content is wrong or corrupted. */
#define AVRC_STS_INTERNAL_ERR                                                  \
    0x03 /* Internal Error, sent if there are error conditions not covered by  \
            a more specific error code. */
#define AVRC_STS_NO_ERROR                                                      \
    0x04 /* Operation completed without error.  This is the status that should \
            be returned if the operation was successful. */
#define AVRC_STS_UID_CHANGED                                                   \
    0x05 /* UID Changed - The UIDs on the device have changed */
/* #define AVRC_STS_GEN_ERROR      0x06    Unknown Error - this is changed to
 * "reserved" */
#define AVRC_STS_BAD_DIR                                                       \
    0x07 /* Invalid Direction - The Direction parameter is invalid - Change    \
            Path*/
#define AVRC_STS_NOT_DIR                                                       \
    0x08 /* Not a Directory - The UID provided does not refer to a folder item \
            Change Path*/
#define AVRC_STS_NOT_EXIST                                                     \
    0x09 /* Does Not Exist - The UID provided does not refer to any item       \
            Change Path, PlayItem, AddToNowPlaying, GetItemAttributes*/
#define AVRC_STS_BAD_SCOPE                                                     \
    0x0a /* Invalid Scope - The scope parameter is invalid  GetFolderItems,    \
            PlayItem, AddToNowPlayer, GetItemAttributes, */
#define AVRC_STS_BAD_RANGE                                                     \
    0x0b /* Range Out of Bounds - The start of range provided is not valid     \
            GetFolderItems*/
#define AVRC_STS_UID_IS_DIR                                                    \
    0x0c /* UID is a Directory - The UID provided refers to a directory, which \
            cannot be handled by this media player   PlayItem, AddToNowPlaying \
          */
#define AVRC_STS_IN_USE                                                        \
    0x0d /* Media in Use - The media is not able to be used for this operation \
            at this time PlayItem, AddToNowPlaying */
#define AVRC_STS_NOW_LIST_FULL                                                 \
    0x0e /* Now Playing List Full - No more items can be added to the Now      \
            Playing List  AddToNowPlaying*/
#define AVRC_STS_SEARCH_NOT_SUP                                                \
    0x0f /* Search Not Supported - The Browsed Media Player does not support   \
            search Search */
#define AVRC_STS_SEARCH_BUSY                                                   \
    0x10 /* Search in Progress - A search operation is already in progress     \
            Search*/
#define AVRC_STS_BAD_PLAYER_ID                                                 \
    0x11 /* Invalid Player Id - The specified Player Id does not refer to a    \
            valid player    SetAddressedPlayer, SetBrowsedPlayer*/
#define AVRC_STS_PLAYER_N_BR                                                   \
    0x12 /* Player Not Browsable - The Player Id supplied refers to a Media    \
            Player which does not support browsing. SetBrowsedPlayer */
#define AVRC_STS_PLAYER_N_ADDR                                                 \
    0x13 /* Player Not Addressed.  The Player Id supplied refers to a player   \
            which is not currently addressed, and the command is not able to   \
            be performed if the player is not set as addressed.   Search,      \
            SetBrowsedPlayer*/
#define AVRC_STS_BAD_SEARCH_RES                                                \
    0x14 /* No valid Search Results - The Search result list does not contain  \
            valid entries, e.g. after being invalidated due to change of       \
            browsed player   GetFolderItems */
#define AVRC_STS_NO_AVAL_PLAYER 0x15 /* No available players ALL */
#define AVRC_STS_ADDR_PLAYER_CHG                                               \
...
#define AVRC_SCOPE_PLAYER_LIST                                                 \
    0x00 /* Media Player Item - Contains all available media players */
#define AVRC_SCOPE_FILE_SYSTEM                                                 \
    0x01 /* Folder Item, Media Element Item                                    \
            - The virtual filesystem containing the media content of the       \
            browsed player */
#define AVRC_SCOPE_SEARCH                                                      \
    0x02 /* Media Element Item  The results of a search operation on the       \
            browsed player */
#define AVRC_SCOPE_NOW_PLAYING                                                 \
    0x03 /* Media Element Item  The Now Playing list (or queue) of the         \
            addressed player */
...
typedef struct {
    ...
    BT_HDR* p_browse_pkt; /* The GKI buffer received. Set to NULL, if the
                             callback function wants to keep the buffer */
} tAVRC_MSG_BROWSE;
...
#define AVRC_IS_VALID_CAP_ID(a)                                                \
    (((a == AVRC_CAP_COMPANY_ID) || (a == AVRC_CAP_EVENTS_SUPPORTED)) ? TRUE   \
                                                                      : FALSE)

#define AVRC_IS_VALID_EVENT_ID(a)                                              \
    (((a >= AVRC_EVT_PLAY_STATUS_CHANGE) &&                                    \
      (a <= AVRC_EVT_APP_SETTING_CHANGE))                                      \
         ? TRUE                                                                \
         : FALSE)
...
#define AVRC_IS_VALID_ATTRIBUTE(a)                                             \
    (((((a > 0) && a <= AVRC_PLAYER_SETTING_SCAN)) ||                          \
      ((a >= AVRC_PLAYER_SETTING_LOW_MENU_EXT) &&                              \
       (a <= AVRC_PLAYER_SETTING_HIGH_MENU_EXT)))                              \
         ? TRUE                                                                \
         : FALSE)

#define AVRC_IS_VALID_MEDIA_ATTRIBUTE(a)                                       \
    ((a >= AVRC_MEDIA_ATTR_ID_TITLE) && (a <= AVRC_MEDIA_ATTR_ID_PLAYING_TIME) \
         ? TRUE                                                                \
         : FALSE)
...
typedef struct {
    ...
    UINT8 major_type;  /* Use AVRC_MJ_TYPE_AUDIO, AVRC_MJ_TYPE_VIDEO,
                          AVRC_MJ_TYPE_BC_AUDIO, or AVRC_MJ_TYPE_BC_VIDEO.*/
    UINT32 sub_type;   /* Use AVRC_SUB_TYPE_NONE, AVRC_SUB_TYPE_AUDIO_BOOK, or
                          AVRC_SUB_TYPE_PODCAST*/
    UINT8 play_status; /* Use AVRC_PLAYSTATE_STOPPED, AVRC_PLAYSTATE_PLAYING,
    ...
    tAVRC_FULL_NAME
        name; /* The player name, name length and character set id.*/
} tAVRC_ITEM_PLAYER;
...
typedef struct {
    ...
    UINT8 type;       /* Use AVRC_FOLDER_TYPE_MIXED, AVRC_FOLDER_TYPE_TITLES,
                         ...
                         AVRC_FOLDER_TYPE_GENRES,       AVRC_FOLDER_TYPE_PLAYLISTS, or
                         AVRC_FOLDER_TYPE_YEARS.*/
    BOOLEAN playable; /* TRUE, if the folder can be played. */
    tAVRC_FULL_NAME
        name; /* The folder name, name length and character set id. */
} tAVRC_ITEM_FOLDER;

typedef struct {
    UINT32 attr_id; /* Use AVRC_MEDIA_ATTR_ID_TITLE, AVRC_MEDIA_ATTR_ID_ARTIST,
...
typedef struct {
    ...
    tAVRC_FULL_NAME
        name;         /* The media name, name length and character set id. */
...
typedef struct {
    UINT8 pdu;
    tAVRC_STS status;
    UINT8 opcode; /* Op Code (copied from avrc_cmd.opcode by AVRC_BldResponse
