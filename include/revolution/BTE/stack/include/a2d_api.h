...
typedef struct {
    ...
    UINT16*
        p_attrs; /* The attributes filter. If NULL, A2DP API sets the attribute
...
/* This data type is used in tA2D_FIND_CBACK to report the result of the SDP
 * discovery process. */
typedef struct {
    UINT16 service_len;    /* Length, in bytes, of the service name */
    UINT16 provider_len;   /* Length, in bytes, of the provider name */
    char* p_service_name;  /* Pointer the service name.  This character string
                            * may not be null terminated.  Use the service_len
                            * parameter to safely copy this string */
    char* p_provider_name; /* Pointer the provider name.  This character string
                            * may not be null terminated. Use the provider_len
                            * parameter to safely copy this string */
...
**
******************************************************************************/
A2D_API extern tA2D_STATUS A2D_AddRecord(UINT16 service_uuid,
                                         char* p_service_name,
                                         char* p_provider_name, UINT16 features,
                                         UINT32 sdp_handle);
