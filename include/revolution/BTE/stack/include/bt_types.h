...
#define BT_EVT_TO_BTU_HCI_BR_EDR_EVT                                           \
    (0x0000 | BT_EVT_TO_BTU_HCI_EVT) /* event from BR/EDR controller */
#define BT_EVT_TO_BTU_HCI_AMP1_EVT                                             \
    (0x0001 | BT_EVT_TO_BTU_HCI_EVT) /* event from local AMP 1 controller */
#define BT_EVT_TO_BTU_HCI_AMP2_EVT                                             \
    (0x0002 | BT_EVT_TO_BTU_HCI_EVT) /* event from local AMP 2 controller */
#define BT_EVT_TO_BTU_HCI_AMP3_EVT                                             \
    (0x0003 | BT_EVT_TO_BTU_HCI_EVT) /* event from local AMP 3 controller */
...
#define BT_EVT_PROXY_INCOMING_MSG                                              \
    0x1A00 /* BlueStackTester event: incoming message from target */
...
#define BT_EVT_TO_NFCCSIM_NCI                                                  \
    0x4a00 /* events to NFCC simulation (NCI packets) */
...
#define STREAM_TO_LAP(a, p)                                                    \
    {                                                                          \
        register int ijk;                                                      \
        register UINT8* plap = (UINT8*)a + LAP_LEN - 1;                        \
        for (ijk = 0; ijk < LAP_LEN; ijk++)                                    \
            *plap-- = *p++;                                                    \
    }
...
#define BE_STREAM_TO_UINT16(u16, p)                                            \
    {                                                                          \
        u16 = (UINT16)(((UINT16)(*(p)) << 8) + (UINT16)(*((p) + 1)));          \
        (p) += 2;                                                              \
    }
...
**
** The lowest 4 bytes byte of the UUID or GUID depends on the feature.
*Typically,
** the value of those bytes will be the PSM or SCN, but it is up to the
*features.
*/
