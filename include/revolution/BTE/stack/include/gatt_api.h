...
#define GATT_CONN_NO_RESOURCES                                                 \
    L2CAP_CONN_NO_RESOURCES /* connection fail for l2cap resource failure */
#define GATT_CONN_TIMEOUT                                                      \
    HCI_ERR_CONNECTION_TOUT /* 0x08 connection timeout                         \
                             */
#define GATT_CONN_TERMINATE_PEER_USER                                          \
    HCI_ERR_PEER_USER /* 0x13 connection terminate by peer user  */
#define GATT_CONN_TERMINATE_LOCAL_HOST                                         \
    HCI_ERR_CONN_CAUSE_LOCAL_HOST /* 0x16 connectionterminated by local host   \
                                   */
...
#define GATT_CONN_LMP_TIMEOUT                                                  \
    HCI_ERR_LMP_RESPONSE_TIMEOUT /* 0x22 connection fail for LMP response tout \
                                  */
#define GATT_CONN_CANCEL                                                       \
    L2CAP_CONN_CANCEL /* 0x0100 L2CAP connection cancelled  */
...
#define GATT_CHAR_BIT_REL_WRITE                                                \
    0x0001 /* permits reliable writes of the Characteristic Value */
#define GATT_CHAR_BIT_WRITE_AUX                                                \
    0x0002 /* permits writes to the characteristic descriptor */
...
typedef struct {
    ...
    UINT16 offset; /* attribute value offset, if no offfset is needed for the
                      command, ignore it */
...
enum { GATT_TRANSPORT_LE, GATT_TRANSPORT_BR_EDR, GATT_TRANSPORT_LE_BR_EDR };
...
#define GATT_LINK_IDLE_TIMEOUT_WHEN_NO_APP                                     \
    0 /* start a idle timer for this duration when no application              \
 ...
 * completed. */
typedef void(tGATT_CMPL_CBACK)(UINT16 conn_id, tGATTC_OPTYPE op,
...
typedef union {
    ...
    UINT8 client_read_index; /* only used for sequential reading client srv chg
                                info */
} tGATTS_SRV_CHG_REQ;
 ...
 */
typedef void(tGATTS_NV_SAVE_CBACK)(BOOLEAN is_saved,
...
**
** Description      This function is called to reserve a block of handles for a
*service.
**
...
**
*******************************************************************************/
GATT_API extern UINT16 GATTS_CreateService(tGATT_IF gatt_if,
                                           tBT_UUID* p_svc_uuid,
...
**
** Parameter        service_handle : To which service this included service is
*added to.
**                  include_svc_handle    : included service handle.
**
** Returns          included service attribute handle. If 0, add included
*service
**                  fail.
**
*******************************************************************************/
...
**                  value declaration into the service database identified by
*the
**                  service handle.
**
** Parameter        service_handle : To which service this included service is
...
**                  into a service database. Add descriptor should follow add
*char
**                  to which it belongs, and next add char should be done only
**                  after all add descriptors for the previous char.
**
** Parameter        service_handle  : To which service this characteristic
*descriptor
**                                    is added to.
**                  perm            : Characteristic value declaration attribute
**                                    permission.
...
**
*******************************************************************************/
GATT_API extern UINT16 GATTS_AddCharDescriptor(UINT16 service_handle,
                                               tGATT_PERM perm,
...
*application
**
** Returns          TRUE if operation succeed, FALSE if handle block was not
*found.
**
*******************************************************************************/
GATT_API extern BOOLEAN
GATTS_DeleteService(tGATT_IF gatt_if, tBT_UUID* p_svc_uuid, UINT16 svc_inst);
...
**                  sup_transport : supported transport(s) for this primary
*service
**
** return           GATT_SUCCESS if sucessfully started; otherwise error code.
**
*******************************************************************************/
GATT_API extern tGATT_STATUS GATTS_StartService(tGATT_IF gatt_if,
...
**
*******************************************************************************/
GATT_API extern tGATT_STATUS GATTC_ExecuteWrite(UINT16 conn_id,
                                                BOOLEAN is_execute);
...
*******************************************************************************/
GATT_API extern BOOLEAN
GATT_GetConnectionInfor(UINT16 conn_id, tGATT_IF* p_gatt_if, BD_ADDR bd_addr);
...
*******************************************************************************/
GATT_API extern BOOLEAN
GATT_GetConnIdIfConnected(tGATT_IF gatt_if, BD_ADDR bd_addr, UINT16* p_conn_id);
