...
#define GAP_ERR_BUSY                                                           \
    (GAP_ERR_GRP + 0x03) /* The requested function was busy                    \
                          */
#define GAP_ERR_NO_CTRL_BLK                                                    \
    (GAP_ERR_GRP + 0x04) /* No control blocks available */
#define GAP_ERR_STARTING_CMD                                                   \
    (GAP_ERR_GRP + 0x05) /* Error occurred while initiating the command */
#define GAP_NO_BDADDR_REC                                                      \
    (GAP_ERR_GRP + 0x06) /* No Inquiry DB record for BD_ADDR */
#define GAP_ERR_ILL_MODE                                                       \
    (GAP_ERR_GRP + 0x07) /* An illegal mode parameter was detected */
#define GAP_ERR_ILL_INQ_TIME                                                   \
    (GAP_ERR_GRP + 0x08) /* An illegal time parameter was detected */
#define GAP_ERR_ILL_PARM                                                       \
    (GAP_ERR_GRP + 0x09) /* An illegal parameter was detected */
#define GAP_ERR_REM_NAME                                                       \
    (GAP_ERR_GRP + 0x0a) /* Error starting the remote device name request */
#define GAP_CMD_INITIATED                                                      \
    (GAP_ERR_GRP + 0x0b) /* The GAP command was started (result pending) */
#define GAP_DEVICE_NOT_UP                                                      \
    (GAP_ERR_GRP +                                                             \
     0x0c) /* The device was not up; the request was not executed */
#define GAP_BAD_BD_ADDR                                                        \
    (GAP_ERR_GRP + 0x0d) /* The bd addr passed in was not found or invalid */

#define GAP_ERR_BAD_HANDLE                                                     \
    (GAP_ERR_GRP + 0x0e) /* Bad GAP handle                       */
#define GAP_ERR_BUF_OFFSET                                                     \
    (GAP_ERR_GRP + 0x0f) /* Buffer offset invalid                */
#define GAP_ERR_BAD_STATE                                                      \
    (GAP_ERR_GRP + 0x10) /* Connection is in invalid state       */
#define GAP_NO_DATA_AVAIL                                                      \
    (GAP_ERR_GRP + 0x11) /* No data available                    */
#define GAP_ERR_CONGESTED                                                      \
    (GAP_ERR_GRP + 0x12) /* BT stack is congested                */
#define GAP_ERR_SECURITY                                                       \
    (GAP_ERR_GRP + 0x13) /* Security failed                      */

#define GAP_ERR_PROCESSING                                                     \
    (GAP_ERR_GRP + 0x14) /* General error processing BTM request */
#define GAP_ERR_TIMEOUT                                                        \
    (GAP_ERR_GRP + 0x15) /* Timeout occurred while processing cmd */
...
#define GAP_CLR_INQUIRY_FILTER                                                 \
    BTM_CLR_INQUIRY_FILTER /* Inquiry Filtering is turned off */
#define GAP_FILTER_COND_DEVICE_CLASS                                           \
    BTM_FILTER_COND_DEVICE_CLASS /* Filter on device class */
#define GAP_FILTER_COND_BD_ADDR                                                \
...
typedef void(tGAP_BLE_DEV_NAME_CBACK)(BOOLEAN status, BD_ADDR addr,
                                      UINT16 length, char* p_name);

typedef void(tGAP_BLE_RECONN_ADDR_CBACK)(BOOLEAN status, BD_ADDR addr,
                                         BD_ADDR reconn_bda);
...
**                  GAP_ERR_PROCESSING if not enough resources to carry out
*request
**
*******************************************************************************/
...
**
*******************************************************************************/
GAP_API extern UINT16 GAP_ReadDiscoverableMode(UINT16* duration,
                                               UINT16* interval);
...
**                  GAP_ERR_ILL_INQ_TIME if a bad interval or duration was
*passed
...
**                  database for the device with the specified name.  If not
*found
**                  it initiates a general inquiry.  Upon completion, it
*retrieves
**                  the name for each device until a match is found or all
*devices
**                  have been checked.  Note:  This process can take a while to
**                  complete.
**
** Returns          BT_PASS if the name was immediately available. (BD_ADDR is
*returned)
...
**
** Description      This function is called to retrieve the current
*connectability
**                  mode for the local device.
**
...
**
*******************************************************************************/
GAP_API extern UINT16 GAP_ReadConnectableMode(UINT16* duration,
                                              UINT16* interval);
...
**
*******************************************************************************/
GAP_API extern UINT16 GAP_SetPairableMode(UINT16 mode,
...
** Function         GAP_PinRsp
**
** Description      This function is called from UI after Security Manager
*submitted
**                  PIN code request.
**
** Returns          void
...
**
** Description      This function retrieves local device information to the
*caller.
**                  NOTE:  Return parameters that are set to NULL are not
*retrieved.
**
** Returns          BT_PASS (0) if successful, or a non-zero error code
**
...
**
** Description      The remote name is retrieved from the specified remote
*device.  If
**                  GAP_CMD_INITIATED is returned by the function, the command
*was
**                  successfully sent to the controller.  The GAP_EVT_NAME_RESP
*event
**                  is passed in the callback when the remote device name has
*been retrieved.
**
** Returns
**                  GAP_CMD_INITIATED if remote search successfully initiated
**                  GAP_ERR_BUSY if a remote name request is already in
*progress,
**                  GAP_ERR_NO_CTRL_BLK if out of control blocks (too many
*commands pending)
...
**
*******************************************************************************/
GAP_API extern UINT16 GAP_GetRemoteDeviceName(BD_ADDR addr,
                                              tGAP_CALLBACK* callback);
...
**
** Description      GKI buffer aware applications can call this function to
*write data
...
**
*******************************************************************************/
GAP_API extern UINT16 GAP_ConnReconfig(UINT16 gap_handle,
...
**
** Returns          UINT16 - maximum size buffer that can be transmitted to the
*peer
**
*******************************************************************************/
...
**
*******************************************************************************/
GAP_API extern void GAP_BleAttrDBUpdate(UINT16 attr_uuid,
...
**
*******************************************************************************/
GAP_API extern BOOLEAN GAP_BleReadPeerDevName(BD_ADDR peer_bda,
                                              tGAP_BLE_DEV_NAME_CBACK* p_cback);
...
**
** Description      Start a process to udpate the reconnect address if remote
*devive
**                  has privacy enabled.
**
** Returns          TRUE if read started, else FALSE if GAP is busy
...
GAP_BleUpdateReconnectAddr(BD_ADDR peer_bda, BD_ADDR reconn_addr,
