 ...
 */
#define L2CAP_FIXED_CHNL_SIG_BIT                                               \
    (1 << L2CAP_SIGNALLING_CID) /* Signal Channel Supported (Mandatory) */
#define L2CAP_FIXED_CHNL_CNCTLESS_BIT                                          \
    (1 << L2CAP_CONNECTIONLESS_CID) /* Connectionless Reception             */
#define L2CAP_FIXED_CHNL_AMP_BIT                                               \
    (1 << L2CAP_AMP_CID) /* AMP Manager Supported                */
#define L2CAP_FIXED_CHNL_ATT_BIT                                               \
    (1 << L2CAP_ATT_CID) /* Attribute protocol Supported         */
#define L2CAP_FIXED_CHNL_BLE_SIG_BIT                                           \
    (1 << L2CAP_BLE_SIGNALLING_CID) /* BLE Signalling Supported             */
#define L2CAP_FIXED_CHNL_SMP_BIT                                               \
    (1 << L2CAP_SMP_CID) /* BLE Security Manager Supported       */
...
#define L2CAP_EXTENDED_FEATURES_INFO_TYPE                                      \
    0x0002 /* Used in Information Req/Response */
#define L2CAP_FIXED_CHANNELS_INFO_TYPE                                         \
    0x0003 /* Used in AMP                      */

#define L2CAP_CONNLESS_MTU_INFO_SIZE 2 /* Connectionless MTU size          */
#define L2CAP_EXTENDED_FEATURES_ARRAY_SIZE                                     \
    4                                 /* Extended features array size     */
#define L2CAP_FIXED_CHNL_ARRAY_SIZE 8 /* Fixed channel array size         */

/* Extended features mask bits
 */
#define L2CAP_EXTFEA_RTRANS                                                    \
    0x00000001                     /* Retransmission Mode (Not Supported)    */
#define L2CAP_EXTFEA_FC 0x00000002 /* Flow Control Mode   (Not Supported) */
#define L2CAP_EXTFEA_QOS 0x00000004
#define L2CAP_EXTFEA_ENH_RETRANS                                               \
    0x00000008 /* Enhanced retransmission mode           */
#define L2CAP_EXTFEA_STREAM_MODE                                               \
    0x00000010 /* Streaming Mode                         */
#define L2CAP_EXTFEA_NO_CRC                                                    \
    0x00000020 /* Optional FCS (if set No FCS desired)   */
#define L2CAP_EXTFEA_EXT_FLOW_SPEC                                             \
    0x00000040 /* Extended flow spec                     */
#define L2CAP_EXTFEA_FIXED_CHNLS                                               \
    0x00000080 /* Fixed channels                         */
#define L2CAP_EXTFEA_EXT_WINDOW                                                \
    0x00000100 /* Extended Window Size                   */
#define L2CAP_EXTFEA_UCD_RECEPTION                                             \
    0x00000200 /* Unicast Connectionless Data Reception  */

/* Mask for locally supported features used in Information Response (default to
 * none) */
...
#define L2CAP_MAX_HEADER_FCS                                                   \
    (L2CAP_PKT_OVERHEAD + L2CAP_EXT_CONTROL_OVERHEAD +                         \
 ...
 */
#define L2CAP_FCR_UNSEG_SDU                                                    \
    0x0000 /* Control word to begin with for unsegmented PDU*/
...
#define L2CAP_FCR_CONT_SDU                                                     \
    0xc000 /* ...for continuation PDU of a segmented SDU */

/* Supervisory frame types
 */
#define L2CAP_FCR_SUP_RR                                                       \
    0x0000 /* Supervisory frame - RR                          */
#define L2CAP_FCR_SUP_REJ                                                      \
    0x0001 /* Supervisory frame - REJ                         */
#define L2CAP_FCR_SUP_RNR                                                      \
    0x0002 /* Supervisory frame - RNR                         */
#define L2CAP_FCR_SUP_SREJ                                                     \
    0x0003 /* Supervisory frame - SREJ                        */

#define L2CAP_FCR_SAR_BITS                                                     \
    0xC000 /* Mask to get the SAR bits from control word */
#define L2CAP_FCR_SAR_BITS_SHIFT                                               \
...
#define L2CAP_FCR_REQ_SEQ_BITS                                                 \
    0x3F00 /* Mask to get the req-seq from control word */
#define L2CAP_FCR_REQ_SEQ_BITS_SHIFT                                           \
    8 /* Bits to shift right to get the req-seq from ctrl-word */
#define L2CAP_FCR_TX_SEQ_BITS                                                  \
    0x007E /* Mask on get the tx-seq from control word */
#define L2CAP_FCR_TX_SEQ_BITS_SHIFT                                            \
    1 /* Bits to shift right to get the tx-seq from ctrl-word */

#define L2CAP_FCR_F_BIT                                                        \
    0x0080 /* F-bit in the control word (Sup and I frames)  */
#define L2CAP_FCR_P_BIT                                                        \
    0x0010 /* P-bit in the control word (Sup frames only)   */

#define L2CAP_FCR_F_BIT_SHIFT 7
#define L2CAP_FCR_P_BIT_SHIFT 4

#define L2CAP_FCR_SEG_BITS                                                     \
    0xC000 /* Mask to get the segmentation bits from ctrl-word */
#define L2CAP_FCR_SUP_SHIFT                                                    \
    2 /* Bits to shift right to get the S-bits from ctrl-word */
#define L2CAP_FCR_SUP_BITS                                                     \
    0x000C /* Mask to get the supervisory bits from ctrl-word */

#define L2CAP_FCR_INIT_CRC 0 /* Initial state of the CRC register */
#define L2CAP_FCR_SEQ_MODULO                                                   \
    0x3F /* Mask for sequence numbers (range 0 - 63)                           \
          */

/*************************************************************************************************
** The following definitions are only used for internal testing of ERTM at the
*application level
*************************************************************************************************/
/* L2CA_SetupErtmTest() corruption test types */
#define L2CAP_FCR_TTYPE_CORR_IFRAME                                            \
    0 /* Corrupt one or more I-frames, based on count   */
#define L2CAP_FCR_TTYPE_CORR_SFRAME                                            \
    1                               /* Corrupt an S-frame, (acknowledgement) */
#define L2CAP_FCR_TTYPE_STOP_TEST 2 /* Used when turning off a test */
#define L2CAP_FCR_TTYPE_GET_CID                                                \
    3 /* Returns RFCOMM cid when '0' is passed in cid argument */
