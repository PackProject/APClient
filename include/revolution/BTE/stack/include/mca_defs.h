...
#define MCA_OP_MDL_CREATE_REQ                                                  \
    0x01 /* create an MDL, wait for an associated data channel connection */
#define MCA_OP_MDL_CREATE_RSP 0x02 /* response to above request */
#define MCA_OP_MDL_RECONNECT_REQ                                               \
    0x03 /* req to prepare to rvc a data channel conn associated with a prev   \
            MDL */
#define MCA_OP_MDL_RECONNECT_RSP 0x04 /* response to above request */
#define MCA_OP_MDL_ABORT_REQ                                                   \
...
#define MCA_OP_SYNC_CAP_REQ                                                    \
    0x11                         /* request sync capabilities & requirements   \
                                  */
...
#define MCA_OP_SYNC_INFO_IND                                                   \
    0x15 /* update of the actual time-stamp clock instant from the sync slave  \
          */
...
/* response code */
#define MCA_RSP_SUCCESS                                                        \
    0x00 /* The corresponding request was received and processed successfully. \
          */
#define MCA_RSP_BAD_OPCODE                                                     \
    0x01 /* The Op Code received is not valid (i.e. neither a Standard Op Code \
            nor a Clock Synchronization Protocol Op Code). */
#define MCA_RSP_BAD_PARAM                                                      \
    0x02 /* One or more of the values in the received request is invalid. */
...
*/
#define MCA_RSP_BAD_MDEP                                                       \
    0x03 /* The MDEP ID referenced does not exist on this device. */
#define MCA_RSP_MDEP_BUSY                                                      \
    0x04 /* The requested MDEP currently has as many active MDLs as it can     \
...
*/
#define MCA_RSP_MDL_BUSY                                                       \
    0x06 /* The device is temporarily unable to complete the request. This is  \
            intended for reasons not related to the physical sensor (e.g.      \
            communication resources unavailable). */
#define MCA_RSP_BAD_OP                                                         \
    0x07 /* The received request is invalid in the current state. */
...
*/
#define MCA_RSP_NO_RESOURCE                                                    \
    0x08 /* The device is temporarily unable to complete the request. This is  \
            intended for reasons relating to the physical sensor (e.g.         \
            hardware fault, low battery), or when processing resources are     \
            temporarily committed to other processes. */
#define MCA_RSP_ERROR                                                          \
    0x09 /* An internal error other than those listed in this table was        \
            encountered while processing the request. */
#define MCA_RSP_NO_SUPPORT                                                     \
    0x0A /* The Op Code that was used in this request is not supported. */
#define MCA_RSP_CFG_REJ                                                        \
    0x0B /* A configuration required by a MD_CREATE_MDL or MD_RECONNECT_MDL    \
