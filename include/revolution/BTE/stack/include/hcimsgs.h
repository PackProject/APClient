...
#define HCIC_GET_UINT8(p, off)                                                 \
    (UINT8)(*((UINT8*)((p) + 1) + p->offset + 3 + (off)))

#define HCIC_GET_UINT16(p, off)                                                \
    (UINT16)((*((UINT8*)((p) + 1) + p->offset + 3 + (off)) +                   \
              (*((UINT8*)((p) + 1) + p->offset + 3 + (off) + 1) << 8)))

#define HCIC_GET_UINT32(p, off)                                                \
    (UINT32)((*((UINT8*)((p) + 1) + p->offset + 3 + (off)) +                   \
              (*((UINT8*)((p) + 1) + p->offset + 3 + (off) + 1) << 8) +        \
...
#define HCIC_GET_ARRAY(p, off, x, len)                                         \
    {                                                                          \
        UINT8* qq = ((UINT8*)((p) + 1) + p->offset + 3 + (off));               \
        UINT8* rr = (UINT8*)x;                                                 \
        int ii;                                                                \
        for (ii = 0; ii < len; ii++)                                           \
            *rr++ = *qq++;                                                     \
...
#define HCIC_GET_BDADDR(p, off, x)                                             \
    {                                                                          \
        UINT8* qq = ((UINT8*)((p) + 1) + p->offset + 3 + (off));               \
        UINT8* rr = (UINT8*)x + BD_ADDR_LEN - 1;                               \
        int ii;                                                                \
...
#define HCIC_GET_DEVCLASS(p, off, x)                                           \
    {                                                                          \
        UINT8* qq = ((UINT8*)((p) + 1) + p->offset + 3 + (off));               \
        UINT8* rr = (UINT8*)x + DEV_CLASS_LEN - 1;                             \
        int ii;                                                                \
        for (ii = 0; ii < DEV_CLASS_LEN; ii++)                                 \
            *rr-- = *qq++;                                                     \
    }
...
#define HCIC_GET_LAP(p, off, x)                                                \
    {                                                                          \
        UINT8* qq = ((UINT8*)((p) + 1) + p->offset + 3 + (off));               \
        UINT8* rr = (UINT8*)x + LAP_LEN - 1;                                   \
        int ii;                                                                \
        for (ii = 0; ii < LAP_LEN; ii++)                                       \
            *rr-- = *qq++;                                                     \
    }
...
btsnd_hcic_per_inq_mode(UINT16 max_period, UINT16 min_period, const LAP inq_lap,
                        UINT8 duration, UINT8 response_cnt);
...
/* Add SCO Connection */
HCI_API extern BOOLEAN btsnd_hcic_add_SCO_conn(UINT16 handle,
                                               UINT16 packet_types);
...
HCI_API extern BOOLEAN btsnd_hcic_change_conn_type(UINT16 handle,
                                                   UINT16 packet_types);
...
HCI_API extern BOOLEAN
btsnd_hcic_auth_request(UINT16 handle); /* Authentication Request */

/* Set Connection Encryption */
HCI_API extern BOOLEAN btsnd_hcic_set_conn_encrypt(UINT16 handle,
                                                   BOOLEAN enable);
...
HCI_API extern BOOLEAN
btsnd_hcic_change_link_key(UINT16 handle); /* Change Connection Link Key */
...
HCI_API extern BOOLEAN
btsnd_hcic_rmt_features_req(UINT16 handle); /* Remote Features Request */

/* Remote Extended Features */
HCI_API extern BOOLEAN btsnd_hcic_rmt_ext_features(UINT16 handle,
                                                   UINT8 page_num);
...
HCI_API extern BOOLEAN
btsnd_hcic_rmt_ver_req(UINT16 handle); /* Remote Version Info Request */
HCI_API extern BOOLEAN
btsnd_hcic_read_rmt_clk_offset(UINT16 handle); /* Remote Clock Offset */
HCI_API extern BOOLEAN
btsnd_hcic_read_lmp_handle(UINT16 handle); /* Remote LMP Handle */

HCI_API extern BOOLEAN
btsnd_hcic_setup_esco_conn(UINT16 handle, UINT32 tx_bw, UINT32 rx_bw,
...
HCI_API extern BOOLEAN
btsnd_hcic_accept_esco_conn(BD_ADDR bd_addr, UINT32 tx_bw, UINT32 rx_bw,
                            UINT16 max_latency, UINT16 content_fmt,
                            UINT8 retrans_effort, UINT16 packet_types);
...
HCI_API extern BOOLEAN btsnd_hcic_reject_esco_conn(BD_ADDR bd_addr,
                                                   UINT8 reason);
...
/* Hold Mode */
HCI_API extern BOOLEAN btsnd_hcic_hold_mode(UINT16 handle,
...
HCI_API extern BOOLEAN
btsnd_hcic_exit_sniff_mode(UINT16 handle); /* Exit Sniff Mode */
...
HCI_API extern BOOLEAN
btsnd_hcic_exit_park_mode(UINT16 handle); /* Exit Park Mode */
...
HCI_API extern BOOLEAN
btsnd_hcic_role_discovery(UINT16 handle); /* Role Discovery */
...
HCI_API extern BOOLEAN
btsnd_hcic_read_policy_set(UINT16 handle); /* Read Policy Settings */

/* Write Policy Settings */
HCI_API extern BOOLEAN btsnd_hcic_write_policy_set(UINT16 handle,
                                                   UINT16 settings);
...
/* Flow Specification */
HCI_API extern BOOLEAN btsnd_hcic_flow_specification(
    UINT16 handle, UINT8 flags, UINT8 flow_direct, UINT8 service_type,
    UINT32 token_rate, UINT32 token_bucket_size, UINT32 peak, UINT32 latency);
...
HCI_API extern void btsnd_hcic_write_ext_inquiry_response(void* buffer,
                                                          UINT8 fec_req);
...
HCI_API extern BOOLEAN
btsnd_hcic_read_ext_inquiry_response(void); /* Read Extended Inquiry Response */
#else
...
HCI_API extern BOOLEAN btsnd_hcic_io_cap_req_reply(BD_ADDR bd_addr,
                                                   UINT8 capability,
                                                   UINT8 oob_present,
                                                   UINT8 auth_req);
...
HCI_API extern BOOLEAN btsnd_hcic_io_cap_req_neg_reply(BD_ADDR bd_addr,
                                                       UINT8 err_code);
...
HCI_API extern BOOLEAN btsnd_hcic_user_conf_reply(BD_ADDR bd_addr,
                                                  BOOLEAN is_yes);
...
HCI_API extern BOOLEAN btsnd_hcic_user_passkey_reply(BD_ADDR bd_addr,
                                                     UINT32 value);
...
HCI_API extern BOOLEAN btsnd_hcic_enhanced_flush(UINT16 handle,
                                                 UINT8 packet_type);

#define HCIC_PARAM_SIZE_ENHANCED_FLUSH 3
#endif

HCI_API extern BOOLEAN btsnd_hcic_send_keypress_notif(BD_ADDR bd_addr,
                                                      UINT8 notif);
...
HCI_API extern BOOLEAN
btsnd_hcic_refresh_encryption_key(UINT16 handle); /* Refresh Encryption Key */

/**** end of Simple Pairing Commands ****/

HCI_API extern BOOLEAN btsnd_hcic_set_event_mask(UINT8 local_controller_id,
                                                 BT_EVENT_MASK evt_mask);
...
/* Reset */
HCI_API extern BOOLEAN
btsnd_hcic_set_event_mask_page_2(UINT8 local_controller_id,
...
HCI_API extern BOOLEAN btsnd_hcic_flush(UINT8 local_controller_id,
                                        UINT16 handle); /* Flush */
...
/* Write Stored Key */
HCI_API extern BOOLEAN btsnd_hcic_write_stored_key(UINT8 num_keys,
                                                   BD_ADDR* bd_addr,
                                                   LINK_KEY* link_key);
...
/* Delete Stored Key */
HCI_API extern BOOLEAN btsnd_hcic_delete_stored_key(BD_ADDR bd_addr,
...
HCI_API extern BOOLEAN
btsnd_hcic_write_pin_type(UINT8 type);                    /* Write PIN Type */
HCI_API extern BOOLEAN btsnd_hcic_read_auto_accept(void); /* Read Auto Accept */
HCI_API extern BOOLEAN
btsnd_hcic_write_auto_accept(UINT8 flag);          /* Write Auto Accept */
HCI_API extern BOOLEAN btsnd_hcic_read_name(void); /* Read Local Name */
HCI_API extern BOOLEAN btsnd_hcic_read_conn_acc_tout(
    UINT8 local_controller_id); /* Read Connection Accept Timout */
HCI_API extern BOOLEAN btsnd_hcic_write_conn_acc_tout(
    UINT8 local_controller_id,
    UINT16 tout); /* Write Connection Accept Timout */
HCI_API extern BOOLEAN btsnd_hcic_read_page_tout(void); /* Read Page Timout */
HCI_API extern BOOLEAN
btsnd_hcic_write_page_tout(UINT16 timeout); /* Write Page Timout */
HCI_API extern BOOLEAN btsnd_hcic_read_scan_enable(void); /* Read Scan Enable */
HCI_API extern BOOLEAN
btsnd_hcic_write_scan_enable(UINT8 flag); /* Write Scan Enable */
HCI_API extern BOOLEAN
btsnd_hcic_read_pagescan_cfg(void); /* Read Page Scan Activity */

HCI_API extern BOOLEAN
btsnd_hcic_write_pagescan_cfg(UINT16 interval,
...
HCI_API extern BOOLEAN
btsnd_hcic_read_inqscan_cfg(void); /* Read Inquiry Scan Activity */

/* Write Inquiry Scan Activity */
HCI_API extern BOOLEAN btsnd_hcic_write_inqscan_cfg(UINT16 interval,
                                                    UINT16 window);
...
HCI_API extern BOOLEAN
btsnd_hcic_read_auth_enable(void); /* Read Authentication Enable */
HCI_API extern BOOLEAN
btsnd_hcic_write_auth_enable(UINT8 flag); /* Write Authentication Enable */
HCI_API extern BOOLEAN
btsnd_hcic_read_encr_mode(void); /* Read encryption mode */
HCI_API extern BOOLEAN
btsnd_hcic_write_encr_mode(UINT8 mode); /* Write encryption mode */
HCI_API extern BOOLEAN
btsnd_hcic_read_dev_class(void); /* Read Class of Device */
HCI_API extern BOOLEAN
btsnd_hcic_write_dev_class(DEV_CLASS dev); /* Write Class of Device */
HCI_API extern BOOLEAN
btsnd_hcic_read_voice_settings(void); /* Read Voice Settings */
HCI_API extern BOOLEAN
btsnd_hcic_write_voice_settings(UINT16 flags); /* Write Voice Settings */
...
HCI_API extern BOOLEAN btsnd_hcic_set_host_flow_ctrl(
    UINT8 value); /* Enable/disable flow control toward host */

HCI_API extern BOOLEAN
btsnd_hcic_read_auto_flush_tout(UINT16 handle); /* Read Retransmit Timout */

HCI_API extern BOOLEAN
btsnd_hcic_write_auto_flush_tout(UINT16 handle,
...
HCI_API extern BOOLEAN
btsnd_hcic_read_num_bcast_xmit(void); /* Read Num Broadcast Retransmits */
HCI_API extern BOOLEAN btsnd_hcic_write_num_bcast_xmit(
    UINT8 num); /* Write Num Broadcast Retransmits */
HCI_API extern BOOLEAN
btsnd_hcic_read_hold_mode_act(void); /* Read Hold Mode Activity */
HCI_API extern BOOLEAN
btsnd_hcic_write_hold_mode_act(UINT8 flags); /* Write Hold Mode Activity */

HCI_API extern BOOLEAN btsnd_hcic_read_tx_power(UINT16 handle,
                                                UINT8 type); /* Read Tx Power */
...
HCI_API extern BOOLEAN
btsnd_hcic_read_sco_flow_enable(void); /* Read Authentication Enable */
HCI_API extern BOOLEAN
btsnd_hcic_write_sco_flow_enable(UINT8 flag); /* Write Authentication Enable */
...
HCI_API extern BOOLEAN
btsnd_hcic_host_num_xmitted_pkts(UINT8 num_handles, UINT16* handle,
...
HCI_API extern BOOLEAN btsnd_hcic_read_link_super_tout(
    UINT8 local_controller_id,
    UINT16 handle); /* Read Link Supervision Timeout */

/* Write Link Supervision Timeout */
HCI_API extern BOOLEAN
btsnd_hcic_write_link_super_tout(UINT8 local_controller_id, UINT16 handle,
                                 UINT16 timeout);
...
HCI_API extern BOOLEAN
btsnd_hcic_read_max_iac(void); /* Read Num Supported IAC */
HCI_API extern BOOLEAN
btsnd_hcic_read_cur_iac_lap(void); /* Read Current IAC LAP */

HCI_API extern BOOLEAN
btsnd_hcic_write_cur_iac_lap(UINT8 num_cur_iac,
...
HCI_API extern BOOLEAN
btsnd_hcic_read_page_scan_per(void); /* Read Page Scan Period Mode */
HCI_API extern BOOLEAN
btsnd_hcic_write_page_scan_per(UINT8 mode); /* Write Page Scan Period Mode */
HCI_API extern BOOLEAN
btsnd_hcic_read_page_scan_mode(void); /* Read Page Scan Mode */
HCI_API extern BOOLEAN
btsnd_hcic_write_page_scan_mode(UINT8 mode); /* Write Page Scan Mode */
HCI_API extern BOOLEAN btsnd_hcic_read_local_ver(
    UINT8 local_controller_id); /* Read Local Version Info */
HCI_API extern BOOLEAN btsnd_hcic_read_local_supported_cmds(
    UINT8 local_controller_id); /* Read Local Supported Commands */
HCI_API extern BOOLEAN
btsnd_hcic_read_local_features(void); /* Read Local Supported Features */
HCI_API extern BOOLEAN
btsnd_hcic_read_buffer_size(void); /* Read Local buffer sizes */
HCI_API extern BOOLEAN
btsnd_hcic_read_country_code(void);                   /* Read Country Code */
HCI_API extern BOOLEAN btsnd_hcic_read_bd_addr(void); /* Read Local BD_ADDR */
HCI_API extern BOOLEAN btsnd_hcic_read_fail_contact_count(
    UINT8 local_controller_id, UINT16 handle); /* Read Failed Contact Counter */
HCI_API extern BOOLEAN btsnd_hcic_reset_fail_contact_count(
    UINT8 local_controller_id,
    UINT16 handle); /* Reset Failed Contact Counter */
HCI_API extern BOOLEAN
btsnd_hcic_get_link_quality(UINT16 handle); /* Get Link Quality */
HCI_API extern BOOLEAN btsnd_hcic_read_rssi(UINT16 handle); /* Read RSSI */
HCI_API extern BOOLEAN
btsnd_hcic_read_loopback_mode(void); /* Read Loopback Mode */
HCI_API extern BOOLEAN
btsnd_hcic_write_loopback_mode(UINT8 mode); /* Write Loopback Mode */
HCI_API extern BOOLEAN
btsnd_hcic_enable_test_mode(void); /* Enable Device Under Test Mode */
HCI_API extern BOOLEAN
btsnd_hcic_write_pagescan_type(UINT8 type); /* Write Page Scan Type */
HCI_API extern BOOLEAN
btsnd_hcic_read_pagescan_type(void); /* Read Page Scan Type */
HCI_API extern BOOLEAN
btsnd_hcic_write_inqscan_type(UINT8 type); /* Write Inquiry Scan Type */
HCI_API extern BOOLEAN
btsnd_hcic_read_inqscan_type(void); /* Read Inquiry Scan Type */
HCI_API extern BOOLEAN
btsnd_hcic_write_inquiry_mode(UINT8 type); /* Write Inquiry Mode */
HCI_API extern BOOLEAN
btsnd_hcic_read_inquiry_mode(void); /* Read Inquiry Mode */
...
HCI_API extern BOOLEAN
btsnd_hcic_set_afh_host_channel_class(UINT8* p_afhchannelmap);
...
/* Send HCI Data */
HCI_API extern void btsnd_hcic_data(BT_HDR* p_buf, UINT16 len, UINT16 handle,
                                    UINT8 boundary, UINT8 broadcast);
...
#define HCID_GET_EVENT(u16)                                                    \
    (UINT8)(((u16) >> HCI_DATA_EVENT_OFFSET) & HCI_DATA_EVENT_MASK)

#define HCI_DATA_BCAST_MASK 3
#define HCI_DATA_BCAST_OFFSET 10
#define HCID_GET_BCAST(u16)                                                    \
    (UINT8)(((u16) >> HCI_DATA_BCAST_OFFSET) & HCI_DATA_BCAST_MASK)

#define HCID_GET_ACL_LEN(p)                                                    \
    (UINT16)((*((UINT8*)((p) + 1) + p->offset + 2) +                           \
...
HCI_API extern void btsnd_hcic_vendor_spec_cmd(void* buffer, UINT16 opcode,
...
HCI_API extern void btsnd_hcie_inq_res(void* buffer, UINT8 num_resp,
                                       UINT8** bd_addr,
                                       UINT8* page_scan_rep_mode,
...
HCI_API extern void btsnd_hcie_connection_comp(void* buffer, UINT8 status,
                                               UINT16 handle, BD_ADDR bd_addr,
                                               UINT8 link_type,
                                               UINT8 encr_mode);
...
HCI_API extern void btsnd_hcie_connection_req(void* buffer, BD_ADDR bd_addr,
                                              DEV_CLASS dev_class,
                                              UINT8 link_type);
...
HCI_API extern void btsnd_hcie_disc_comp(void* buffer, UINT8 status,
                                         UINT16 handle, UINT8 reason);

#define HCIE_PARAM_SIZE_DISC_COMP 4

/* Authentication Complete Event */
HCI_API extern void btsnd_hcie_auth_comp(void* buffer, UINT8 status,
                                         UINT16 handle);

#define HCIE_PARAM_SIZE_AUTH_COMP 3

/* Remote Name Request Complete Event */
HCI_API extern void btsnd_hcie_rmt_name_req_comp(void* buffer, UINT8 status,
                                                 BD_ADDR bd_addr, BD_NAME name);

#define HCIE_PARAM_SIZE_RMT_NAME_REQ_COMP (1 + BD_ADDR_LEN + BD_NAME_LEN)

/* Encryption Change Event */
HCI_API extern void btsnd_hcie_encryption_change(void* buffer, UINT8 status,
                                                 UINT16 handle, BOOLEAN enable);

#define HCIE_PARAM_SIZE_ENCR_CHANGE 4

/* Connection Link Key Change Event */
HCI_API extern void btsnd_hcie_conn_link_key_change(void* buffer, UINT8 status,
                                                    UINT16 handle);

#define HCIE_PARAM_SIZE_LINK_KEY_CHANGE 3

/* Encryption Key Refresh Complete Event */
HCI_API extern void btsnd_hcie_encrypt_key_refresh(void* buffer, UINT8 status,
                                                   UINT16 handle);

#define HCIE_PARAM_SIZE_ENCRYPT_KEY_REFRESH 3

/* Master Link Key Complete Event */
HCI_API extern void btsnd_hcie_master_link_key(void* buffer, UINT8 status,
                                               UINT16 handle, UINT8 flag);

#define HCIE_PARAM_SIZE_MASTER_LINK_KEY 4

/* Read Remote Supported Features Complete Event */
HCI_API extern void btsnd_hcie_read_rmt_features(void* buffer, UINT8 status,
                                                 UINT16 handle,
                                                 UINT8* features);
...
HCI_API extern void btsnd_hcie_read_rmt_ext_features(void* buffer, UINT8 status,
                                                     UINT16 handle,
                                                     UINT8 page_num,
                                                     UINT8 max_page_num,
                                                     UINT8* features);
...
HCI_API extern void btsnd_hcie_read_rmt_version(void* buffer, UINT8 status,
                                                UINT16 handle, UINT8 version,
                                                UINT16 comp_name,
                                                UINT16 sub_version);
...
/* QOS setup complete */
HCI_API extern void
btsnd_hcie_qos_setup_compl(void* buffer, UINT8 status, UINT16 handle,
                           UINT8 flags, UINT8 service_type, UINT32 token_rate,
                           UINT32 peak, UINT32 latency, UINT32 delay_var);

#define HCIE_PARAM_SIZE_QOS_SETUP_COMP 21

/* Flow Specification complete */
HCI_API extern void btsnd_hcie_flow_spec_compl(
    void* buffer, UINT8 status, UINT16 handle, UINT8 flags,
    UINT8 flow_direction, UINT8 service_type, UINT32 token_rate,
    UINT32 token_bucket_size, UINT32 peak, UINT32 latency);

#define HCIE_PARAM_SIZE_FLOW_SPEC_COMP 22

/*  Command Complete Event */
HCI_API extern void btsnd_hcie_cmd_comp(void* buffer, UINT8 max_host_cmds,
                                        UINT16 opcode, UINT8 status);

#define HCIE_PARAM_SIZE_CMD_COMP 4

/*  Command Complete with pre-filled in parameters */
HCI_API extern void btsnd_hcie_cmd_comp_params(void* buffer,
                                               UINT8 max_host_cmds,
                                               UINT16 cmd_opcode, UINT8 status);

#define HCI_CMD_COMPL_PARAM_OFFSET 4

/*  Command Complete Event with 1-byte param */
HCI_API extern void btsnd_hcie_cmd_comp_param1(void* buffer,
                                               UINT8 max_host_cmds,
                                               UINT16 opcode, UINT8 status,
                                               UINT8 param1);
...
HCI_API extern void btsnd_hcie_cmd_comp_param2(void* buffer,
                                               UINT8 max_host_cmds,
                                               UINT16 opcode, UINT8 status,
                                               UINT16 param2);
...
HCI_API extern void btsnd_hcie_cmd_comp_bd_addr(void* buffer,
                                                UINT8 max_host_cmds,
                                                UINT16 opcode, UINT8 status,
                                                BD_ADDR bd_addr);
...
HCI_API extern void btsnd_hcie_cmd_status(void* buffer, UINT8 status,
                                          UINT8 max_host_cmds, UINT16 opcode);
...
/*  Role Changed Event */
HCI_API extern void btsnd_hcie_role_change(void* buffer, UINT8 status,
                                           BD_ADDR bd_addr, UINT8 role);

#define HCIE_PARAM_SIZE_ROLE_CHANGE 8

/* Ready for Data Packets Event */
HCI_API extern void btsnd_hcie_num_compl_pkts(void* buffer, UINT8 num_handles,
                                              UINT16* p_handle,
                                              UINT16* num_pkts);
...
/* Mode Change Event */
HCI_API extern void btsnd_hcie_mode_change(void* buffer, UINT8 status,
                                           UINT16 handle, UINT8 mode,
                                           UINT16 interval);
...
HCI_API extern void btsnd_hcie_return_link_keys(void* buffer, UINT8 num_keys,
                                                BD_ADDR* bd_addr,
                                                LINK_KEY* link_key);
...
HCI_API extern void btsnd_hcie_link_key_notify(void* buffer, BD_ADDR bd_addr,
                                               LINK_KEY link_key,
                                               UINT8 key_type);
...
HCI_API extern void btsnd_hcie_loopback_command(void* buffer, UINT8 data_len,
                                                UINT8* data);
...
HCI_API extern void btsnd_hcie_max_slots_change(void* buffer, UINT16 handle,
                                                UINT8 max_slots);

#define HCIE_PARAM_SIZE_MAX_SLOTS_CHANGE 3

/* Read Clock Offset Complet Event */
HCI_API extern void btsnd_hcie_read_clock_off_comp(void* buffer, UINT8 status,
                                                   UINT16 handle,
                                                   UINT16 clock_offset);
...
HCI_API extern void btsnd_hcie_pkt_type_change(void* buffer, UINT8 status,
                                               UINT16 handle, UINT16 pkt_type);
...
HCI_API extern void btsnd_hcie_pagescan_mode_chng(void* buffer, BD_ADDR bd_addr,
                                                  UINT8 mode);
...
HCI_API extern void
btsnd_hcie_pagescan_rep_mode_chng(void* buffer, BD_ADDR bd_addr, UINT8 mode);

#define HCIE_PARAM_SIZE_PAGE_SCAN_REP_MODE_CHNG 7

/* Sniff Sub Rate Event */
HCI_API extern void btsnd_hcie_sniff_sub_rate(void* buffer, UINT8 status,
                                              UINT16 handle, UINT16 max_tx_lat,
                                              UINT16 max_rx_lat,
...
HCI_API extern void
btsnd_hcie_ext_inquiry_result(void* buffer, UINT8 num_resp, UINT8** bd_addr,
                              UINT8* page_scan_rep_mode, UINT8* reserved,
                              UINT8** dev_class, UINT16* clock_offset,
                              UINT8* rssi, UINT8* p_data);

#if (BLE_INCLUDED == TRUE)
/********************************************************************************
** BLE Commands
**      Note: "local_controller_id" is for transport, not counted in HCI message
*size
*********************************************************************************/
...
HCI_API extern BOOLEAN
btsnd_hcic_ble_write_adv_params(UINT16 adv_int_min, UINT16 adv_int_max,
...
HCI_API extern BOOLEAN btsnd_hcic_ble_set_adv_data(UINT8 data_len,
                                                   UINT8* p_data);

HCI_API extern BOOLEAN btsnd_hcic_ble_set_scan_rsp_data(UINT8 data_len,
                                                        UINT8* p_scan_rsp);
...
HCI_API extern BOOLEAN btsnd_hcic_ble_set_scan_enable(UINT8 scan_enable,
                                                      UINT8 duplicate);

HCI_API extern BOOLEAN btsnd_hcic_ble_create_ll_conn(
    UINT16 scan_int, UINT16 scan_win, UINT8 init_filter_policy,
    UINT8 addr_type_peer, BD_ADDR bda_peer, UINT8 addr_type_own,
    UINT16 conn_int_min, UINT16 conn_int_max, UINT16 conn_latency,
    UINT16 conn_timeout, UINT16 min_ce_len, UINT16 max_ce_len);
...
HCI_API extern BOOLEAN btsnd_hcic_ble_add_white_list(UINT8 addr_type,
                                                     BD_ADDR bda);

HCI_API extern BOOLEAN btsnd_hcic_ble_remove_from_white_list(UINT8 addr_type,
                                                             BD_ADDR bda);

HCI_API extern BOOLEAN btsnd_hcic_ble_upd_ll_conn_params(
    UINT16 handle, UINT16 conn_int_min, UINT16 conn_int_max,
    UINT16 conn_latency, UINT16 conn_timeout, UINT16 min_len, UINT16 max_len);

HCI_API extern BOOLEAN
btsnd_hcic_ble_set_host_chnl_class(UINT8 chnl_map[HCIC_BLE_CHNL_MAP_SIZE]);
...
HCI_API extern BOOLEAN btsnd_hcic_ble_encrypt(UINT8* key, UINT8 key_len,
...
HCI_API extern BOOLEAN
btsnd_hcic_ble_start_enc(UINT16 handle, UINT8 rand[HCIC_BLE_RAND_DI_SIZE],
                         UINT16 ediv, UINT8 ltk[HCIC_BLE_ENCRYT_KEY_SIZE]);

HCI_API extern BOOLEAN
btsnd_hcic_ble_ltk_req_reply(UINT16 handle,
                             UINT8 ltk[HCIC_BLE_ENCRYT_KEY_SIZE]);
