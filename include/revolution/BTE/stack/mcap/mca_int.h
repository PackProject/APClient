...
enum {
    ...
    MCA_CCB_MSG_RSP_EVT, /* Response received event.  This event is sent
                            whenever a response message is received for an
    ...
    MCA_CCB_LL_CLOSE_EVT, /* Lower layer close.  This event is sent when the
                             lower layer channel is closed. */
...
enum {
    ...
    MCA_DCB_TC_DATA_EVT   /* This event is sent when a data packet is received
                             from the peer.*/
};
...
typedef struct {
    ...
    UINT16 lcid;     /* L2CAP LCID */
    UINT8 tcid; /* transport channel id (0, for control channel. (MDEP ID + 1)
                   for data channel) */
    tMCA_DL cb_idx;  /* 1-based index to ccb or dcb */
    UINT8 state;     /* transport channel state */
    UINT8 cfg_flags; /* L2CAP configuration flags */
    UINT8 id; /* L2CAP id sent by peer device (need this to handle security
                 pending) */
} tMCA_TC_TBL;
...
enum {
    ...
    MCA_CCB_STAT_RECONN,  /* reinitiate connection after transitioning from
...
typedef struct {
    ...
    UINT16 ctrl_vpsm;     /* The virtual PSM that peer is listening for control
                             channel */
    UINT16 data_vpsm;     /* The virtual PSM that peer is listening for data
                             channel. */
 ...
 * dcb[0]                             ...dcb[MCA_NUM_MDLS*1*MCA_NUM_LINKS*1-1]
 * -> rcb[0]
 * dcb[MCA_NUM_MDLS*1*MCA_NUM_LINKS*1]...dcb[MCA_NUM_MDLS*2*MCA_NUM_LINKS*2-1]
 * -> rcb[1]
 * dcb[MCA_NUM_MDLS*2*MCA_NUM_LINKS*2]...dcb[MCA_NUM_MDLS*3*MCA_NUM_LINKS*3-1]
 * -> rcb[2]
 */
typedef struct {
    tMCA_CCB* p_ccb; /* the associated control control block */
    BT_HDR* p_data;  /* data packet held due to L2CAP channel congestion */
    tMCA_CS* p_cs;   /* the associated MDEP info. p_cs->type is the mdep
                        id(internal use) */
...
/* cact functions */
extern void mca_ccb_report_event(tMCA_CCB* p_ccb, UINT8 event,
                                 tMCA_CTRL* p_data);
...
/* l2c functions */
extern void mca_l2c_cconn_ind_cback(BD_ADDR bd_addr, UINT16 lcid, UINT16 psm,
                                    UINT8 id);
extern void mca_l2c_dconn_ind_cback(BD_ADDR bd_addr, UINT16 lcid, UINT16 psm,
                                    UINT8 id);
extern UINT16 mca_l2c_open_req(BD_ADDR bd_addr, UINT16 PSM,
