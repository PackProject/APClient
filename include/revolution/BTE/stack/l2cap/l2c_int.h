...
#ifndef L2CAP_CHNL_CONNECT_TOUT /* BTIF needs to override for internal project \
                                   needs */
...
#define L2CEVT_LP_QOS_VIOLATION_IND                                            \
    6 /* Lower layer QOS violation indication                                  \
       */
...
#define L2CEVT_L2CAP_CONNECT_RSP_NEG                                           \
    13                             /* Peer connection response (failed)    */
...
#define L2CEVT_L2CA_CONNECT_RSP_NEG                                            \
    23                            /* Upper layer connect response (failed)*/
...
#define L2CEVT_L2CA_CONFIG_RSP_NEG                                             \
    26                                /* Upper layer config response (failed)  \
                                       */
...
#define L2CAP_MIN_RETRANS_TOUT                                                 \
    2000 /* Min retransmission timeout if no flush timeout or PBF */
#define L2CAP_MIN_MONITOR_TOUT                                                 \
    12000 /* Min monitor timeout if no flush timeout or PBF */
...
typedef struct {
    ...
    UINT16 count; /* How many I-frames to drop in a row; used only with one-shot
                     tests */
} tL2C_FCR_TEST_CFG;
...
#define CCB_FLAG_SENT_PENDING 0x02 /* already sent pending response */
    ...
    BOOLEAN out_cfg_fcr_present; /* TRUE if cfg response shoulkd include fcr
                                    options */
...
/* It will make sure that low priority channel (for example, HF signaling on
 * RFCOMM)      */
/* can be sent to headset even if higher priority channel (for example, AV media
 * channel) */
/* is congested. */
...
#define UPD_PENDING                                                            \
    2 /* while updates are disabled, peer requested new parameters */
...
typedef struct {
...
#if (L2CAP_NON_FLUSHABLE_PB_INCLUDED == TRUE)
    UINT16 non_flushable_pbf; /* L2CAP_PKT_START_NON_FLUSHABLE if controller
                                 supports */
    ...
    tL2CA_ECHO_DATA_CB* p_echo_data_cb; /* Echo data callback */
     ...
     (L2CAP_HIGH_PRI_CHAN_QUOTA_IS_CONFIGURABLE == TRUE))
    UINT16 high_pri_min_xmit_quota; /* Minimum number of ACL credit for high
                                       priority link */
 ...
 */
#if (!defined(L2CAP_HIGH_PRI_CHAN_QUOTA_IS_CONFIGURABLE) ||                    \
     (L2CAP_HIGH_PRI_CHAN_QUOTA_IS_CONFIGURABLE == FALSE))
...
extern BOOLEAN l2cu_set_acl_priority(BD_ADDR bd_addr, UINT8 priority,
                                     BOOLEAN reset_after_rs);
...
extern void l2cu_send_peer_connect_rsp(tL2C_CCB* p_ccb, UINT16 result,
                                       UINT16 status);
...
extern void l2cu_send_peer_echo_req(tL2C_LCB* p_lcb, UINT8* p_data,
                                    UINT16 data_len);
extern void l2cu_send_peer_echo_rsp(tL2C_LCB* p_lcb, UINT8 id, UINT8* p_data,
                                    UINT16 data_len);
extern void l2cu_send_peer_info_rsp(tL2C_LCB* p_lcb, UINT8 id,
                                    UINT16 info_type);
extern void l2cu_reject_connection(tL2C_LCB* p_lcb, UINT16 remote_cid,
                                   UINT8 rem_id, UINT16 result);
...
#if (BLE_INCLUDED == TRUE)
extern void l2cu_send_peer_ble_par_req(tL2C_LCB* p_lcb, UINT16 min_int,
                                       UINT16 max_int, UINT16 latency,
                                       UINT16 timeout);
extern void l2cu_send_peer_ble_par_rsp(tL2C_LCB* p_lcb, UINT16 reason,
                                       UINT8 rem_id);
#endif
...
****************************************
*/
extern BOOLEAN l2cu_check_feature_req(tL2C_LCB* p_lcb, UINT8 id, UINT8* p_data,
                                      UINT16 data_len);
extern void l2cu_check_feature_rsp(tL2C_LCB* p_lcb, UINT8 id, UINT8* p_data,
                                   UINT16 data_len);
...
extern BOOLEAN l2c_link_hci_conn_comp(UINT8 status, UINT16 handle,
                                      BD_ADDR p_bda);
...
extern void l2c_link_process_num_completed_blocks(UINT8 controller_id, UINT8* p,
                                                  UINT16 evt_len);
...
L2C_API extern BT_HDR* l2cap_link_chk_pkt_start(
    BT_HDR*); /* Called at start of rcv to check L2CAP segmentation */
L2C_API extern BOOLEAN l2cap_link_chk_pkt_end(
    void); /* Called at end   of rcv to check L2CAP segmentation */
...
extern void l2c_fcr_send_S_frame(tL2C_CCB* p_ccb, UINT16 function_code,
                                 UINT16 pf_bit);
extern BT_HDR* l2c_fcr_clone_buf(BT_HDR* p_buf, UINT16 new_offset,
                                 UINT16 no_of_bytes, UINT8 pool);
...
extern BOOLEAN l2c_fcr_adj_our_req_options(tL2C_CCB* p_ccb,
                                           tL2CAP_CFG_INFO* p_cfg);
extern void l2c_fcr_adj_our_rsp_options(tL2C_CCB* p_ccb,
                                        tL2CAP_CFG_INFO* p_peer_cfg);
extern BOOLEAN l2c_fcr_renegotiate_chan(tL2C_CCB* p_ccb,
                                        tL2CAP_CFG_INFO* p_cfg);
extern UINT8 l2c_fcr_process_peer_cfg_req(tL2C_CCB* p_ccb,
                                          tL2CAP_CFG_INFO* p_cfg);
...
extern void l2cble_conn_comp(UINT16 handle, UINT8 role, BD_ADDR bda,
                             tBLE_ADDR_TYPE type, UINT16 conn_interval,
