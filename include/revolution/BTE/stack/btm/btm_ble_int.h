...
#define BTM_BLE_VALID_PRAM(x, min, max)                                        \
...
typedef struct {
    ...
    UINT16 br_edr_supported_flag; /* combined BR EDR discoverable and
                                     connectable mode */
...
extern void btm_ble_connected(UINT8* bda, UINT16 handle, UINT8 enc_mode,
                              UINT8 role);
...
extern void btm_ble_ltk_request_reply(BD_ADDR bda, BOOLEAN use_stk,
                                      BT_OCTET16 stk);
extern UINT8 btm_proc_smp_cback(tSMP_EVT event, BD_ADDR bd_addr,
                                tSMP_EVT_DATA* p_data);
extern tBTM_STATUS btm_ble_set_encryption(BD_ADDR bd_addr, void* p_ref_data,
                                          UINT8 link_role);
extern void btm_ble_ltk_request(UINT16 handle, UINT8 rand[8], UINT16 ediv);
extern BOOLEAN btm_ble_start_encrypt(BD_ADDR bda, BOOLEAN use_stk,
                                     BT_OCTET16 stk);
...
extern void btm_ble_rand_enc_complete(UINT8* p, UINT16 op_code,
...
extern BOOLEAN btm_ble_resume_bg_conn(tBTM_BLE_SEL_CBACK* p_sele_callback,
                                      BOOLEAN def_param);
...
extern BOOLEAN btm_ble_start_select_conn(BOOLEAN start,
                                         tBTM_BLE_SEL_CBACK* p_select_cback);
...
BT_API extern void btm_ble_set_test_mac_value(BOOLEAN enable,
                                              UINT8* p_test_mac_val);
BT_API extern void
btm_ble_set_test_local_sign_cntr_value(BOOLEAN enable,
