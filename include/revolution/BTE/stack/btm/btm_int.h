...
#define BTM_EPR_AVAILABLE(p)                                                   \
    ((HCI_ATOMIC_ENCRYPT_SUPPORTED((p)->features) &&                           \
      HCI_ATOMIC_ENCRYPT_SUPPORTED(btm_cb.devcb.local_features))               \
         ? TRUE                                                                \
         : FALSE)

#define BTM_IS_BRCM_CONTROLLER()                                               \
    (btm_cb.devcb.local_version.manufacturer == LMP_COMPID_BROADCOM)
...
typedef struct {
    ...
    UINT16 restore_pkt_types; /* when coming in/out of SCO connection restore
                                 the packet types */
    UINT16 clock_offset;
    BD_ADDR remote_addr;
    DEV_CLASS remote_dc;
...
typedef struct {
    ...
    tBTM_VS_EVT_CB*
        p_vend_spec_cb[BTM_MAX_VSE_CALLBACKS]; /* Register for vendor specific
                                                  events  */
...
#define BTM_RMT_NAME_INQ 0x4 /* Remote name initiated internally by inquiry */
    ...
    tBTM_CMPL_CB* p_inqfilter_cmpl_cb; /* Called (if not NULL) after inquiry
                                          filter completed */
    tBTM_INQ_DB_CHANGE_CB*
        p_inq_change_cb; /* Inquiry database changed callback    */
    ...
    tBTM_INQ_PARMS
        inqparms; /* Contains the parameters for the current inquiry */
    tBTM_INQUIRY_CMPL inq_cmpl_info; /* Status and number of responses from the
                                        last inquiry */
    ...
    UINT8 pending_filt_complete_event; /* to take care of
                                          btm_event_filter_complete
                                          corresponding to */
                                       /* inquiry that has been cancelled*/
    UINT8 inqfilt_type; /* Contains the inquiry filter type (BD ADDR, COD, or
                           Clear) */
...
#define BTM_SCO_2_ESCO(scotype)                                                \
    ((UINT16)(((scotype) & BTM_SCO_PKT_TYPE_MASK) >> 5))
...
typedef struct {
    ...
    UINT16 div;          /* local DIV  to generate local LTK=d1(ER,DIV,0) and
                            CSRK=d1(ER,DIV,1)  */
    ...
    UINT8 local_csrk_sec_level; /* security property of local CSRK for this
                                   device */
...
typedef struct {
    ...
    UINT32 trusted_mask[BTM_SEC_SERVICE_ARRAY_SIZE]; /* Bitwise OR of trusted
                                                        services     */
...
#define BTM_SEC_STATE_DELAY_FOR_ENC                                            \
    7 /* delay to check for encryption to work around controller problems */
    ...
    BOOLEAN link_key_not_sent; /* link key notification has not been sent
                                  waiting for name */
...
#define BTM_SEC_IS_SM4_LEGACY(sm)                                              \
    ((BOOLEAN)(BTM_SM4_KNOWN == ((sm) & BTM_SM4_TRUE)))
#define BTM_SEC_IS_SM4_UNKNOWN(sm)                                             \
    ((BOOLEAN)(BTM_SM4_UNKNOWN == ((sm) & BTM_SM4_TRUE)))
...
typedef struct {
    tBTM_PM_PWR_MD
        req_mode[BTM_MAX_PM_RECORDS +
                 1]; /* the desired mode and parameters of the connection*/
    tBTM_PM_PWR_MD set_mode; /* the mode and parameters sent down to the host
                                controller. */
...
enum {
    ...
    BTM_PAIR_STATE_WAIT_LOCAL_OOB_RSP, /* Waiting for local response to peer OOB
                                          data  */
    BTM_PAIR_STATE_WAIT_LOCAL_IOCAPS,  /* Waiting for local IO capabilities and
                                          OOB data */
...
#if (defined(BTM_BUSY_LEVEL_CHANGE_INCLUDED) &&                                \
     BTM_BUSY_LEVEL_CHANGE_INCLUDED == TRUE)
    tBTM_BL_EVENT_MASK bl_evt_mask;
    tBTM_BL_CHANGE_CB*
        p_bl_changed_cb; /* Callback for when Busy Level changed */
#else
    tBTM_ACL_DB_CHANGE_CB*
        p_acl_changed_cb; /* Callback for when ACL DB changed */
#endif

    tBTM_LSTO_CBACK*
        p_lsto_cback; /* for link supervision timeout change event */
...
// btla-specific ++
    ...
    BOOLEAN paging;
    BOOLEAN discing;
    BUFFER_Q sec_pending_q; /* pending sequrity requests in tBTM_SEC_QUEUE_ENTRY
                               format */
...
extern tBTM_STATUS btm_initiate_rem_name(BD_ADDR remote_bda,
                                         tBTM_INQ_INFO* p_cur, UINT8 origin,
...
extern void btm_process_clk_off_comp_evt(UINT16 hci_handle,
                                         UINT16 clock_offset);
extern void btm_acl_role_changed(UINT8 hci_status, BD_ADDR bd_addr,
                                 UINT8 new_role);
...
#if BTM_PWR_MGR_INCLUDED == FALSE
extern void btm_process_mode_change(UINT8 hci_status, UINT16 hci_handle,
...
extern void btm_esco_proc_conn_chg(UINT8 status, UINT16 handle,
                                   UINT8 tx_interval, UINT8 retrans_window,
                                   UINT16 rx_pkt_len, UINT16 tx_pkt_len);
...
extern tBTM_STATUS btm_sec_l2cap_access_req(BD_ADDR bd_addr, UINT16 psm,
                                            ...
                                            void* p_ref_data);
extern tBTM_STATUS
btm_sec_mx_access_request(BD_ADDR bd_addr, UINT16 psm, BOOLEAN is_originator,
...
extern void btm_sec_encrypt_change(UINT16 handle, UINT8 status,
                                   UINT8 encr_enable);
extern void btm_sec_connected(UINT8* bda, UINT16 handle, UINT8 status,
                              UINT8 enc_mode);
...
extern void btm_sec_link_key_notification(UINT8* p_bda, UINT8* p_link_key,
                                          UINT8 key_type);
...
extern BOOLEAN btm_sec_find_bonded_dev(UINT8 start_idx, UINT8* p_found_idx,
