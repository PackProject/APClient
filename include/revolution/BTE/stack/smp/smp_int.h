...
/* auumption it's only using the low 8 bits, if bigger than that, need to expand
 * it to be 16 bits */
...
SMP_API extern void smp_set_test_confirm_value(BOOLEAN enable,
                                               UINT8* p_c_value);
...
extern BOOLEAN smp_encrypt_data(UINT8* key, UINT8 key_len, UINT8* plain_text,
