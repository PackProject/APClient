...
typedef struct {
    ...
    char application_name[BTIF_HL_APPLICATION_NAME_LEN +
                          1]; /* applicaiton name */
} btif_hl_app_cb_t;
...
extern BOOLEAN btif_hl_find_mcl_idx(UINT8 app_idx, BD_ADDR p_bd_addr,
                                    UINT8* p_mcl_idx);
...
extern BOOLEAN btif_hl_find_mcl_idx_using_handle(tBTA_HL_MCL_HANDLE mcl_handle,
                                                 UINT8* p_app_idx,
                                                 UINT8* p_mcl_idx);
extern BOOLEAN btif_hl_save_mdl_cfg(UINT8 app_id, UINT8 item_idx,
