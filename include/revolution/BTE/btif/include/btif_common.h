...
#ifndef LOG_TAG
#error                                                                         \
    "LOG_TAG not defined, please add in .c file prior to including bt_common.h"
#endif
...
#define ASSERTC(cond, msg, val)                                                \
    if (!(cond)) {                                                             \
        ALOGE("### ASSERT : %s line %d %s (%d) ###", __FILE__, __LINE__, msg,  \
              val);                                                            \
    }
...
enum {
    ...
    BTIF_PAN_CB_DISCONNECTING, /* PAN Disconnect has been sent to BTA
                                  successfully */
};
