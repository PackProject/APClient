...
#define BTIF_AV_FEAT_METADATA                                                  \
    BTA_AV_FEAT_METADATA /* remote control Metadata Transfer command/response  \
                          */
...
#define BTIF_AV_FEEDING_ASYNCHRONOUS                                           \
    0 /* asynchronous feeding, use tx av timer */
#define BTIF_AV_FEEDING_SYNCHRONOUS                                            \
    1 /* synchronous feeding, no av tx timer                                   \
       */

#define BTIF_AV_MAX_SYNCHRONOUS_LATENCY                                        \
    80 /* max latency in ms for BTIF_AV_FEEDING_SYNCHRONOUS */
#define BTIF_AV_MIN_SYNCHRONOUS_LATENCY                                        \
    4 /* min latency in ms for BTIF_AV_FEEDING_SYNCHRONOUS */
...
typedef struct {
    ...
    UINT8
    info[AVDT_CODEC_SIZE]; /* Codec info (can be config or capabilities) */
} tBTIF_AV_CODEC_INFO;
