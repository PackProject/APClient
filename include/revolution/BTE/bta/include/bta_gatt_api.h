...
#define BTA_GATTC_WRITE_CHAR_EVT                                               \
    4 /* GATT write characteristic or char descriptor event */
...
#define BTA_GATTC_READ_DESCR_EVT                                               \
    8 /* GATT read characterisitc descriptor event */
#define BTA_GATTC_WRITE_DESCR_EVT                                              \
    9                          /* GATT write characteristic descriptor event */
...
#define BTA_GATT_CONN_NO_RESOURCES                                             \
    GATT_CONN_NO_RESOURCES /* connection fail for l2cap resource failure */
...
typedef union {
    ...
    tBTA_GATTS_ADD_RESULT
        add_result; /* add included service: BTA_GATTS_ADD_INCL_SRVC_EVT
...
**
*******************************************************************************/
BTA_API extern void BTA_GATTC_CancelOpen(tBTA_GATTC_IF client_if,
                                         BD_ADDR remote_bda, BOOLEAN is_direct);
...
**
*******************************************************************************/
BTA_API extern void BTA_GATTC_ServiceSearchRequest(UINT16 conn_id,
...
**
** Description      This function is called to find the first charatceristic of
*the
**                  service on the given server.
**
** Parameters       conn_id: connection ID which identify the server.
**                  p_srvc_id: the service ID of which the characteristic is
*belonged to.
**                  p_char_uuid_cond: Characteristic UUID, if NULL find the
*first available
**                               characteristic.
**                  p_char_result: output parameter which will store the GATT
**                                  characteristic ID.
**                  p_property: output parameter to carry the characteristic
*property.
**
** Returns          returns status.
**
*******************************************************************************/
BTA_API extern tBTA_GATT_STATUS BTA_GATTC_GetFirstChar(
    UINT16 conn_id, tBTA_GATT_SRVC_ID* p_srvc_id, tBT_UUID* p_char_uuid_cond,
    tBTA_GATTC_CHAR_ID* p_char_result, tBTA_GATT_CHAR_PROP* p_property);

/*******************************************************************************
**
** Function         BTA_GATTC_GetNextChar
**
** Description      This function is called to find the next charatceristic of
*the
**                  service on the given server.
**
** Parameters       conn_id: connection ID which identify the server.
**                  p_start_char_id: start the characteristic search from the
*next record
...
BTA_GATTC_GetNextChar(UINT16 conn_id, tBTA_GATTC_CHAR_ID* p_start_char_id,
...
**
** Description      This function is called to find the first charatceristic
*descriptor of the
**                  charatceristic on the given server.
**
** Parameters       conn_id: connection ID which identify the server.
**                  p_char_id: the characteristic ID of which the descriptor is
*belonged to.
**                  p_descr_uuid_cond: Characteristic Descr UUID, if NULL find
*the first available
**                               characteristic.
...
*******************************************************************************/
BTA_API extern tBTA_GATT_STATUS
BTA_GATTC_GetFirstCharDescr(UINT16 conn_id, tBTA_GATTC_CHAR_ID* p_char_id,
...
**
** Parameters       conn_id: connection ID which identify the server.
**                  p_start_descr_id: start the characteristic search from the
*next record
**                           after the one identified by p_start_descr_id.
**                  p_descr_uuid_cond: Characteristic descriptor UUID, if NULL
*find
...
**
*******************************************************************************/
BTA_API extern tBTA_GATT_STATUS BTA_GATTC_GetNextCharDescr(
    UINT16 conn_id, tBTA_GATTC_CHAR_DESCR_ID* p_start_descr_id,
    tBT_UUID* p_descr_uuid_cond, tBTA_GATTC_CHAR_DESCR_ID* p_descr_result);

/*******************************************************************************
**
** Function         BTA_GATTC_GetFirstIncludedService
**
** Description      This function is called to find the first included service
*of the
**                  service on the given server.
**
** Parameters       conn_id: connection ID which identify the server.
**                  p_srvc_id: the service ID of which the included service is
*belonged to.
**                  p_uuid_cond: include service UUID, if NULL find the first
*available
**                               included service.
...
**
** Description      This function is called to find the next included service of
*the
**                  service on the given server.
**
** Parameters       conn_id: connection ID which identify the server.
...
**                  p_uuid_cond: Included service UUID, if NULL find the first
*available
**                               included service.
**                  p_result: output parameter which will store the GATT ID
**                              of the included service found.
...
**
*******************************************************************************/
BTA_API extern tBTA_GATT_STATUS BTA_GATTC_GetNextIncludedService(
    UINT16 conn_id, tBTA_GATTC_INCL_SVC_ID* p_start_id, tBT_UUID* p_uuid_cond,
    tBTA_GATTC_INCL_SVC_ID* p_result);
...
**
** Description      This function is called to read a service's characteristics
*of
...
*******************************************************************************/
BTA_API extern void
BTA_GATTC_ReadCharDescr(UINT16 conn_id,
...
**
** Description      This function is called to write characteristic descriptor
*value.
**
** Parameters       conn_id - connection ID
...
*******************************************************************************/
BTA_API extern void
BTA_GATTC_WriteCharDescr(UINT16 conn_id,
                         tBTA_GATTC_CHAR_DESCR_ID* p_char_descr_id,
                         tBTA_GATTC_WRITE_TYPE write_type,
                         tBTA_GATT_UNFMT* p_data, tBTA_GATT_AUTH_REQ auth_req);
...
**
*******************************************************************************/
BTA_API extern void BTA_GATTC_SendIndConfirm(UINT16 conn_id,
                                             tBTA_GATTC_CHAR_ID* p_char_id);
...
*******************************************************************************/
BTA_API extern tBTA_GATT_STATUS
BTA_GATTC_RegisterForNotifications(tBTA_GATTC_IF client_if, BD_ADDR remote_bda,
                                   tBTA_GATTC_CHAR_ID* p_char_id);
...
**
*******************************************************************************/
BTA_API extern tBTA_GATT_STATUS BTA_GATTC_DeregisterForNotifications(
    tBTA_GATTC_IF client_if, BD_ADDR remote_bda, tBTA_GATTC_CHAR_ID* p_char_id);
...
**
*******************************************************************************/
BTA_API extern void BTA_GATTC_ReadMultiple(UINT16 conn_id,
...
**
*******************************************************************************/
BTA_API extern void BTA_GATTS_AppRegister(tBT_UUID* p_app_uuid,
...
**
*******************************************************************************/
BTA_API extern void BTA_GATTS_CreateService(tBTA_GATTS_IF server_if,
                                            tBT_UUID* p_service_uuid,
                                            UINT8 inst, UINT16 num_handle,
                                            BOOLEAN is_primary);
...
**
** Description      This function is called to add an included service. After
*included
**                  service is included, a callback event
*BTA_GATTS_ADD_INCL_SRVC_EVT
...
**
*******************************************************************************/
BTA_API extern void BTA_GATTS_AddIncludeService(UINT16 service_id,
...
**
** Description      This function is called to add a characteristic into a
*service.
**
** Parameters       service_id: service ID to which this included service is to
**                              be added.
...
**
*******************************************************************************/
BTA_API extern void BTA_GATTS_AddCharacteristic(UINT16 service_id,
...
**
** Description      This function is called to add characteristic descriptor.
*When
**                  it's done, a callback event BTA_GATTS_ADD_DESCR_EVT is
*called
**                  to report the status and an ID number for this descriptor.
**
** Parameters       service_id: service ID to which this charatceristic
*descriptor is to
**                              be added.
**                  perm: descriptor access permission.
**                  p_descr_uuid: descriptor UUID.
**                  p_descr_params: descriptor value if it's read only
*descriptor.
**
** Returns          returns status.
**
...
**
*******************************************************************************/
BTA_API extern void BTA_GATTS_StartService(UINT16 service_id,
                                           tBTA_GATT_TRANSPORT sup_transport);
...
**
*******************************************************************************/
BTA_API extern void BTA_GATTS_CancelOpen(tBTA_GATTS_IF server_if,
                                         BD_ADDR remote_bda, BOOLEAN is_direct);
