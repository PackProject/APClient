 ...
 * bta_service_id_to_btm_srv_id_lkup_tbl table in bta_dm_act.c.
 */
...
#define BTA_DM_GENERAL_DISC                                                    \
    BTM_GENERAL_DISCOVERABLE /* General discoverable.                          \
                              */
#define BTA_DM_LIMITED_DISC                                                    \
    BTM_LIMITED_DISCOVERABLE /* Limited discoverable.                          \
                              */
...
enum {
    ...
    BTA_DM_PARTIAL_SCATTERNET, /* Device supports partial scatternet. It can
                                  have simulateous connection in Master and
                                  Slave roles for short period of time */
...
typedef struct {
    ...
    UINT8 max_resps;    /* Maximum inquiry responses.  Set to zero for unlimited
                           responses. */
...
typedef struct {
    UINT8 bta_dm_eir_min_name_len; /* minimum length of local name when it is
                                      shortened */
    ...
    UINT8 bta_dm_eir_manufac_spec_len; /* length of manufacturer specific in
                                          bytes */
...
#define BTA_DM_AUTH_SMP_PASSKEY_FAIL                                           \
    BTA_DM_AUTH_CONVERT_SMP_CODE(SMP_PASSKEY_ENTRY_FAIL)
...
typedef BOOLEAN(tBTA_DM_BLE_SEL_CBACK)(BD_ADDR random_bda,
                                       UINT8* p_remote_name);
...
#define BTA_AUTH_SP_NO                                                         \
    BTM_AUTH_SP_NO /* 0 MITM Protection Not Required - Single                  \
                     Profile/non-bonding Numeric comparison with automatic     \
                     accept allowed */
...
#define BTA_AUTH_AP_YES                                                        \
    BTM_AUTH_AP_YES /* 3 MITM Protection Required - All Profiles/dedicated     \
                      bonding Use IO Capabilities to determine authentication  \
                      procedure */
#define BTA_AUTH_SPGB_NO                                                       \
    BTM_AUTH_SPGB_NO /* 4 MITM Protection Not Required - Single                \
                       Profiles/general bonding Numeric comparison with        \
                       automatic accept allowed */
#define BTA_AUTH_SPGB_YES                                                      \
    BTM_AUTH_SPGB_YES /* 5 MITM Protection Required - Single Profiles/general  \
                        bonding Use IO Capabilities to determine               \
...
typedef struct {
    ...
    DEV_CLASS dev_class; /* peer CoD */
    BD_NAME bd_name;     /* peer device name */
    UINT32 passkey; /* the numeric value for comparison. If just_works, do not
                       show this number to UI */
...
#ifndef REVOLUTION
    BOOLEAN remt_name_not_required; /* Application sets this flag if it already
                                       knows the name of the device */
...
*******************************************************************************/
#ifdef REVOLUTION
BTA_API extern void BTA_DmSetVisibility(tBTA_DM_DISC disc_mode,
...
**
*******************************************************************************/
BTA_API extern void BTA_DmSetScanParam(UINT16 page_scan_interval,
...
**                  In order to use this API, BTM_BYPASS_AMP_AUTO_AFH must be
*set
**                  to be TRUE
**
** Returns          void
...
**
*******************************************************************************/
BTA_API extern tBTA_STATUS
BTA_DmVendorSpecificCommand(UINT16 opcode, UINT8 param_len, UINT8* p_param_buf,
...
**
*******************************************************************************/
BTA_API extern void BTA_DmSearch(tBTA_DM_INQ* p_dm_inq,
...
**
*******************************************************************************/
BTA_API extern void BTA_DmPinReply(BD_ADDR bd_addr, BOOLEAN accept,
...
**
*******************************************************************************/
BTA_API extern void BTA_DmLinkPolicy(BD_ADDR bd_addr,
                                     tBTA_DM_LP_MASK policy_mask, BOOLEAN set);
...
**
*******************************************************************************/
BTA_API extern void BTA_DmAuthorizeReply(BD_ADDR bd_addr,
...
**
*******************************************************************************/
BTA_API extern void BTA_DmSignalStrength(tBTA_SIG_STRENGTH_MASK mask,
                                         UINT16 period, BOOLEAN start);
...
**
** Description      This function is called to override the BTA default EIR
*parameters.
...
**
*******************************************************************************/
BTA_API extern UINT8* BTA_CheckEirData(UINT8* p_eir, UINT8 tag,
                                       UINT8* p_length);
...
**
*******************************************************************************/
BTA_API extern void BTA_GetEirService(UINT8* p_eir,
...
**
** Description      This function is called to check if the connected peer
*device
**                  supports SSR or not.
**
...
*******************************************************************************/
BTA_API extern tBTA_STATUS
BTA_DmGetLocalDiRecord(tBTA_DI_GET_RECORD* p_device_info, UINT32* p_handle);
...
**
*******************************************************************************/
BTA_API extern void BTA_DmDiDiscover(BD_ADDR remote_device,
                                     tBTA_DISCOVERY_DB* p_db, UINT32 len,
                                     tBTA_DM_SEARCH_CBACK* p_cback);
...
**
*******************************************************************************/
BTA_API extern void bta_dmexecutecallback(tBTA_DM_EXEC_CBACK* p_callback,
                                          void* p_param);
...
**
*******************************************************************************/
BTA_API extern void BTA_DmPcmInitSamples(UINT32 src_sps, UINT32 bits,
                                         UINT32 n_channels);
...
**
**************************************************************************************/
BTA_API extern INT32 BTA_DmPcmResample(void* p_src, UINT32 in_bytes,
                                       void* p_dst);
#endif
...
**
*******************************************************************************/
BTA_API extern void BTA_DmBleSecurityGrant(BD_ADDR bd_addr,
...
*******************************************************************************/
BTA_API extern void
BTA_DmBleSetBgConnType(tBTA_DM_BLE_CONN_TYPE bg_conn_type,
...
**
*******************************************************************************/
BTA_API extern void BTA_DmBlePasskeyReply(BD_ADDR bd_addr, BOOLEAN accept,
                                          UINT32 passkey);
...
**
*******************************************************************************/
BTA_API extern void BTA_DmAddBleDevice(BD_ADDR bd_addr,
...
**
*******************************************************************************/
BTA_API extern void BTA_DmSearchExt(tBTA_DM_INQ* p_dm_inq,
...
**
*******************************************************************************/
BTA_API extern void BTA_DmDiscoverExt(BD_ADDR bd_addr,
                                      tBTA_SERVICE_MASK_EXT* p_services,
                                      tBTA_DM_SEARCH_CBACK* p_cback,
                                      BOOLEAN sdp_search);
...
**
*******************************************************************************/
BTA_API extern void BTA_DmSetEncryption(BD_ADDR bd_addr,
