...
#define BTA_FS_CO_ENOSPACE                                                     \
    GOEP_ENOSPACE /* Returned in bta_fs_ci_open if no room */
#define BTA_FS_CO_EIS_DIR GOEP_EIS_DIR
#define BTA_FS_CO_RESUME GOEP_RESUME /* used in ci_open, on resume */
#define BTA_FS_CO_NONE                                                         \
    GOEP_NONE /* used in ci_open, on resume (no file to resume) */
 ...
 */
typedef struct {
    char path[BTA_FS_CO_PATH_LEN +
              1]; /* the "current path". path[0]==0-> root */
    char file[BTA_FS_CO_PATH_LEN + 1]; /* file[0] !=0 on resume -> the previous
                                          suspended session had opened files */
...
*applicable)
**                  evt     - event that must be passed into the call-in
*function.
**                  app_id  - application ID specified in the enable functions.
**                            It can be used to identify which profile is the
*caller
**                            of the call-out function.
**
** Returns          void
**
**                  Note: Upon completion of the request, a file descriptor
*(int),
**                        if successful, and an error code (tBTA_FS_CO_STATUS)
**                        are returned in the call-in function,
*bta_fs_ci_open().
**
*******************************************************************************/
...
**                  established (p_sess_info != NULL) or ended (p_sess_info ==
*NULL).
**
...
**
*******************************************************************************/
BTA_API extern void bta_fs_co_session_info(BD_ADDR bd_addr, UINT8* p_sess_info,
                                           UINT8 ssn, tBTA_FS_CO_SESS_ST new_st,
                                           char* p_path, UINT8* p_info,
                                           UINT8 app_id);
...
**
*******************************************************************************/
BTA_API extern void bta_fs_co_resume_op(UINT32 offset, UINT16 evt,
                                        UINT8 app_id);
...
**                  info    - the BTA specific information (like last active
*operation).
**                  p_offset- the location to receive object offset of the
*suspended session
**                  app_id  - application ID specified in the enable functions.
**                            It can be used to identify which profile is the
*caller
...
**
*******************************************************************************/
BTA_API extern void bta_fs_co_suspend(BD_ADDR bd_addr, UINT8* p_sess_info,
                                      UINT8 ssn, UINT32* p_timeout,
                                      UINT32* p_offset, UINT8 info,
                                      UINT8 app_id);
...
**                  This is used to retrieve the session ID and related
*information
**
** Parameters       evt     - event that must be passed into the call-in
*function.
**                  app_id  - application ID specified in the enable functions.
**                            It can be used to identify which profile is the
*caller
...
** Returns          void
**
**                  Note: Upon completion of the request, the related session
*information,
**                        if successful, and an error code (tBTA_FS_CO_STATUS)
**                        are returned in the call-in function,
*bta_fs_ci_resume().
...
**
** Description      This function is executed by BTA when resuming a session.
**                  This is used to inform call-out module if the ssn/file
*offset
...
**                  evt     - event that must be passed into the call-in
*function.
**                  ssn     - session sequence number. Ignored, if
*bta_fs_co_open *							  was
*not called with BTA_FS_CO_RELIABLE.
**                  app_id  - application ID specified in the enable functions.
**                            It can be used to identify which profile is the
*caller
**                            of the call-out function.
...
**
*******************************************************************************/
BTA_API extern void bta_fs_co_read(int fd, UINT8* p_buf, UINT16 nbytes,
                                   UINT16 evt, UINT8 ssn, UINT8 app_id);
...
**
*******************************************************************************/
BTA_API extern void bta_fs_co_write(int fd, const UINT8* p_buf, UINT16 nbytes,
                                    UINT16 evt, UINT8 ssn, UINT8 app_id);
...
**                  origin  - Initial position: BTA_FS_SEEK_SET,
*BTA_FS_SEEK_CUR,
**                            or BTA_FS_SEEK_END.
**
** Returns          void
**
*******************************************************************************/
BTA_API extern void bta_fs_co_seek(int fd, INT32 offset, INT16 origin,
                                   UINT8 app_id);
...
**                  directory.
**
** Parameters       p_path   - (input) file or directory to access (fully
*qualified path).
**                  mode     - (input) [BTA_FS_ACC_EXIST, BTA_FS_ACC_READ, or
*BTA_FS_ACC_RDWR]
**                  p_is_dir - (output) returns TRUE if p_path specifies a
*directory.
**                  app_id   - (input) application ID specified in the enable
*functions.
**                                     It can be used to identify which profile
*is the caller
**                                     of the call-out function.
**
** Returns          (tBTA_FS_CO_STATUS) status of the call.
...
**
*******************************************************************************/
BTA_API extern tBTA_FS_CO_STATUS
bta_fs_co_access(const char* p_path, int mode, BOOLEAN* p_is_dir, UINT8 app_id);
...
**                  the pathname given by path. The pathname is a null
*terminated
**                  string. All components of the path must already exist.
**
** Parameters       p_path   - (input) name of directory to create (fully
*qualified path).
**                  app_id   - (input) application ID specified in the enable
*functions.
**                                     It can be used to identify which profile
...
**
*******************************************************************************/
BTA_API extern tBTA_FS_CO_STATUS bta_fs_co_mkdir(const char* p_path,
                                                 UINT8 app_id);
...
**
** Parameters       p_path   - (input) name of directory to remove (fully
*qualified path).
**                  app_id   - (input) application ID specified in the enable
*functions.
**                                     It can be used to identify which profile
...
**
*******************************************************************************/
BTA_API extern tBTA_FS_CO_STATUS bta_fs_co_rmdir(const char* p_path,
                                                 UINT8 app_id);
...
**
*******************************************************************************/
BTA_API extern tBTA_FS_CO_STATUS bta_fs_co_unlink(const char* p_path,
                                                  UINT8 app_id);
...
**
** Description      This function is called to retrieve a directory entry for
*the
...
**                  p_entry (input/output) - Points to last entry data (valid
*when
**                                           first_item is FALSE)
**                  evt     - event that must be passed into the call-in
*function.
...
**                        BTA_FS_CO_EODIR is returned when no more entries
*[finished]
**                        BTA_FS_CO_FAIL is returned if an error occurred
**
*******************************************************************************/
BTA_API extern void bta_fs_co_getdirentry(const char* p_path,
                                          BOOLEAN first_item,
...
**
** Parameters       p_src_path  - (input) name of file/directory to be copied
*(fully qualified path).
**                  p_dest_path - (input) new name of file/directory(fully
*qualified path).
...
**                      [BTA_FS_CO_EACCES if p_dest_path already exists or could
*not be created (invalid path);
**                                        or p_src_path is a directory and
*p_dest_path specifies a different path. ]
**                      [BTA_FS_CO_FAIL otherwise]
**
*******************************************************************************/
BTA_API extern void bta_fs_co_copy(const char* p_src_path,
                                   const char* p_dest_path, UINT8* p_perms,
                                   UINT16 evt, UINT8 app_id);
...
**                  name is given by p_src_path to p_dest_path.
**
** Parameters       p_src_path  - (input) name of file/directory to be moved
*(fully qualified path).
**                  p_dest_path - (input) new name of file/directory(fully
*qualified path).
**                  p_perms     - the permission of the new object.
...
**
*******************************************************************************/
BTA_API extern void bta_fs_co_rename(const char* p_src_path,
                                     const char* p_dest_path, UINT8* p_perms,
                                     UINT16 evt, UINT8 app_id);
...
**
** Description      This function is called to set the permission a
*file/directory
**                  with name as p_src_path.
**
** Parameters       p_src_path  - (input) name of file/directory to set
*permission (fully qualified path).
...
**
*******************************************************************************/
BTA_API extern void bta_fs_co_set_perms(const char* p_src_path, UINT8* p_perms,
                                        UINT16 evt, UINT8 app_id);
...
**
*******************************************************************************/
BTA_API extern void bta_fs_co_sess_fopen(const char* p_path, int oflags,
                                         UINT8 app_id);
...
**
*******************************************************************************/
BTA_API extern void bta_fs_co_sess_offset(UINT8 ssn, INT32 pos, UINT16 nbytes,
                                          UINT8 app_id);
...
**
** Description      find the active session control block for the given an
*app_id.
**
** Returns          the control block found.
**
