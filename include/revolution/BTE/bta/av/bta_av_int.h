...
#define BTA_AV_CO_ID_TO_BE_STREAM(p, u32)                                      \
    {                                                                          \
        *(p)++ = (UINT8)((u32) >> 16);                                         \
        *(p)++ = (UINT8)((u32) >> 8);                                          \
        *(p)++ = (UINT8)(u32);                                                 \
...
typedef UINT8 (*tBTA_AV_CO_GETCFG)(tBTA_AV_HNDL hndl, tBTA_AV_CODEC codec_type,
                                   UINT8* p_codec_info, UINT8* p_sep_info_idx,
...
typedef void (*tBTA_AV_CO_OPEN)(tBTA_AV_HNDL hndl, tBTA_AV_CODEC codec_type,
...
typedef void* (*tBTA_AV_CO_DATAPATH)(tBTA_AV_CODEC codec_type, UINT32* p_len,
                                     UINT32* p_timestamp);
...
typedef struct {
    const tBTA_AV_ACT*
        p_act_tbl; /* the action table for stream state machine */
    ...
    UINT16 cur_psc_mask; /* Protocol service capabilities mask for current
                            connection */
    ...
    BOOLEAN recfg_sup;    /* TRUE if the first attempt to reconfigure the stream
                             was successfull, else False if command fails */
    BOOLEAN suspend_sup;  /* TRUE if Suspend stream is supported, else FALSE if
...
extern void bta_av_conn_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event,
                              tAVDT_CTRL* p_data);
extern UINT8 bta_av_rc_create(tBTA_AV_CB* p_cb, UINT8 role, UINT8 shdl,
                              UINT8 lidx);
extern void bta_av_proc_stream_evt(UINT8 handle, BD_ADDR bd_addr, UINT8 event,
                                   tAVDT_CTRL* p_data, int index);
...
extern void bta_av_sm_execute(tBTA_AV_CB* p_cb, UINT16 event,
                              tBTA_AV_DATA* p_data);
extern void bta_av_ssm_execute(tBTA_AV_SCB* p_scb, UINT16 event,
                               tBTA_AV_DATA* p_data);
