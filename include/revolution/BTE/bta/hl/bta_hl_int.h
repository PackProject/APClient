...
#define BTA_HL_L2C_USER_TX_POOL_ID                                             \
    L2CAP_DEFAULT_ERM_POOL_ID /* todo this should be based on data type */
...
typedef struct {
    ...
    char srv_desp[BTA_SERVICE_DESP_LEN + 1]; /* service description to be used
                                                in the SDP; null terminated */
    char
        provider_name[BTA_PROVIDER_NAME_LEN + 1]; /* provide name to be used in
...
#define BTA_HL_GET_BUF_PTR(p_pkt)                                              \
    ((UINT8*)((UINT8*)(p_pkt + 1) + p_pkt->offset))
...
/* sdp */
extern BOOLEAN
bta_hl_fill_sup_feature_list(const tSDP_DISC_ATTR* p_attr,
                             tBTA_HL_SUP_FEATURE_LIST_ELEM* p_list);

extern tBTA_HL_STATUS bta_hl_sdp_register(UINT8 app_idx);
extern tSDP_DISC_REC*
bta_hl_find_sink_or_src_srv_class_in_db(const tSDP_DISCOVERY_DB* p_db,
                                        const tSDP_DISC_REC* p_start_rec);

/* action routines */
extern void bta_hl_dch_ci_get_tx_data(UINT8 app_idx, UINT8 mcl_idx,
                                      UINT8 mdl_idx, tBTA_HL_DATA* p_data);
extern void bta_hl_dch_ci_put_rx_data(UINT8 app_idx, UINT8 mcl_idx,
                                      UINT8 mdl_idx, tBTA_HL_DATA* p_data);
extern void bta_hl_dch_ci_get_echo_data(UINT8 app_idx, UINT8 mcl_idx,
                                        UINT8 mdl_idx, tBTA_HL_DATA* p_data);

extern void bta_hl_dch_echo_test(UINT8 app_idx, UINT8 mcl_idx, UINT8 mdl_idx,
                                 tBTA_HL_DATA* p_data);
extern void bta_hl_dch_ci_put_echo_data(UINT8 app_idx, UINT8 mcl_idx,
                                        UINT8 mdl_idx, tBTA_HL_DATA* p_data);
...
extern void bta_hl_dch_sdp_fail(UINT8 app_idx, UINT8 mcl_idx, UINT8 mdl_idx,
                                tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_cong_change(UINT8 app_idx, UINT8 mcl_idx,
                                       UINT8 mdl_idx, tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_reconnect_ind(UINT8 app_idx, UINT8 mcl_idx,
                                         UINT8 mdl_idx, tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_reconnect_cfm(UINT8 app_idx, UINT8 mcl_idx,
                                         UINT8 mdl_idx, tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_reconnect(UINT8 app_idx, UINT8 mcl_idx,
                                     UINT8 mdl_idx, tBTA_HL_DATA* p_data);
extern void bta_hl_dch_sdp_init(UINT8 app_idx, UINT8 mcl_idx, UINT8 mdl_idx,
                                tBTA_HL_DATA* p_data);
extern void bta_hl_dch_close_echo_test(UINT8 app_idx, UINT8 mcl_idx,
                                       UINT8 mdl_idx, tBTA_HL_DATA* p_data);
...
extern void bta_hl_dch_close_cmpl(UINT8 app_idx, UINT8 mcl_idx, UINT8 mdl_idx,
                                  tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_close_cfm(UINT8 app_idx, UINT8 mcl_idx,
                                     UINT8 mdl_idx, tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_close_ind(UINT8 app_idx, UINT8 mcl_idx,
                                     UINT8 mdl_idx, tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_close(UINT8 app_idx, UINT8 mcl_idx, UINT8 mdl_idx,
                                 tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_delete_ind(UINT8 app_idx, UINT8 mcl_idx,
                                      UINT8 mdl_idx, tBTA_HL_DATA* p_data);

extern void bta_hl_dch_mca_delete_cfm(UINT8 app_idx, UINT8 mcl_idx,
                                      UINT8 mdl_idx, tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_delete(UINT8 app_idx, UINT8 mcl_idx, UINT8 mdl_idx,
                                  tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_abort_ind(UINT8 app_idx, UINT8 mcl_idx,
                                     UINT8 mdl_idx, tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_abort_cfm(UINT8 app_idx, UINT8 mcl_idx,
                                     UINT8 mdl_idx, tBTA_HL_DATA* p_data);
...
extern void bta_hl_dch_mca_open_cfm(UINT8 app_idx, UINT8 mcl_idx, UINT8 mdl_idx,
                                    tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_create_ind(UINT8 app_idx, UINT8 mcl_idx,
                                      UINT8 mdl_idx, tBTA_HL_DATA* p_data);
extern void bta_hl_dch_mca_create_cfm(UINT8 app_idx, UINT8 mcl_idx,
                                      UINT8 mdl_idx, tBTA_HL_DATA* p_data);
...
extern tSDP_DISC_CMPL_CB*
bta_hl_allocate_spd_cback(tBTA_HL_SDP_OPER sdp_oper, UINT8 app_idx,
                          UINT8 mcl_idx, UINT8 mdl_idx, UINT8* p_sdp_cback_idx);
extern tBTA_HL_STATUS bta_hl_init_sdp(tBTA_HL_SDP_OPER sdp_oper, UINT8 app_idx,
                                      UINT8 mcl_idx, UINT8 mdl_idx);
extern void bta_hl_cch_sdp_init(UINT8 app_idx, UINT8 mcl_idx,
                                tBTA_HL_DATA* p_data);
extern void bta_hl_cch_mca_open(UINT8 app_idx, UINT8 mcl_idx,
                                tBTA_HL_DATA* p_data);
extern void bta_hl_cch_mca_close(UINT8 app_idx, UINT8 mcl_idx,
                                 tBTA_HL_DATA* p_data);
extern void bta_hl_cch_close_cmpl(UINT8 app_idx, UINT8 mcl_idx,
                                  tBTA_HL_DATA* p_data);
extern void bta_hl_cch_mca_disconnect(UINT8 app_idx, UINT8 mcl_idx,
                                      tBTA_HL_DATA* p_data);
extern void bta_hl_cch_mca_rsp_tout(UINT8 app_idx, UINT8 mcl_idx,
                                    tBTA_HL_DATA* p_data);
extern void bta_hl_cch_mca_connect(UINT8 app_idx, UINT8 mcl_idx,
                                   tBTA_HL_DATA* p_data);

/* State machine drivers  */
extern void bta_hl_cch_sm_execute(UINT8 inst_idx, UINT8 mcl_idx, UINT16 event,
                                  tBTA_HL_DATA* p_data);
...
extern UINT16 bta_hl_get_service_uuids(UINT8 sdp_oper, UINT8 app_idx,
                                       UINT8 mcl_idx, UINT8 mdl_idx);
extern BOOLEAN bta_hl_find_echo_cfg_rsp(UINT8 app_idx, UINT8 mcl_idx,
                                        UINT8 mdep_idx, UINT8 cfg,
                                        UINT8* p_cfg_rsp);
extern BOOLEAN bta_hl_validate_cfg(UINT8 app_idx, UINT8 mcl_idx, UINT8 mdl_idx,
                                   UINT8 cfg);
extern BOOLEAN bta_hl_find_cch_cb_indexes(tBTA_HL_DATA* p_msg, UINT8* p_app_idx,
                                          UINT8* p_mcl_idx);
extern BOOLEAN bta_hl_find_dch_cb_indexes(tBTA_HL_DATA* p_msg, UINT8* p_app_idx,
                                          UINT8* p_mcl_idx, UINT8* p_mdl_idx);
extern UINT16 bta_hl_allocate_mdl_id(UINT8 app_idx, UINT8 mcl_idx,
                                     UINT8 mdl_idx);
...
extern BOOLEAN bta_hl_find_dch_setup_mdl_idx(UINT8 app_idx, UINT8 mcl_idx,
                                             UINT8* p_mdl_idx);
extern BOOLEAN bta_hl_find_an_in_use_mcl_idx(UINT8 app_idx, UINT8* p_mcl_idx);
...
extern BOOLEAN bta_hl_find_mcl_idx_using_handle(tBTA_HL_MCL_HANDLE mcl_handle,
                                                UINT8* p_app_idx,
                                                UINT8* p_mcl_idx);
extern BOOLEAN bta_hl_find_mcl_idx(UINT8 app_idx, BD_ADDR p_bd_addr,
                                   UINT8* p_mcl_idx);
extern BOOLEAN bta_hl_is_the_first_reliable_existed(UINT8 app_idx,
                                                    UINT8 mcl_idx);
extern BOOLEAN bta_hl_find_non_active_mdl_cfg(UINT8 app_idx,
                                              UINT8 start_mdl_cfg_idx,
                                              UINT8* p_mdl_cfg_idx);
...
extern BOOLEAN bta_hl_validate_peer_cfg(UINT8 app_idx, UINT8 mcl_idx,
                                        UINT8 mdl_idx,
                                        tBTA_HL_MDEP_ID peer_mdep_id,
                                        tBTA_HL_MDEP_ROLE peer_mdep_role,
                                        UINT8 sdp_idx);
...
extern BOOLEAN
bta_hl_validate_reconnect_params(UINT8 app_idx, UINT8 mcl_idx,
...
extern void bta_hl_set_dch_chan_cfg(UINT8 app_idx, UINT8 mcl_idx, UINT8 mdl_idx,
                                    tBTA_HL_DATA* p_data);
extern BOOLEAN bta_hl_get_l2cap_cfg(tBTA_HL_MDL_HANDLE mdl_hnd,
                                    tBTA_HL_L2CAP_CFG_INFO* p_cfg);
extern BOOLEAN bta_hl_validate_chan_cfg(UINT8 app_idx, UINT8 mcl_idx,
                                        UINT8 mdl_idx);
extern BOOLEAN bta_hl_is_cong_on(UINT8 app_id, BD_ADDR bd_addr,
                                 tBTA_HL_MDL_ID mdl_id);
extern void bta_hl_check_cch_close(UINT8 app_idx, UINT8 mcl_idx,
                                   tBTA_HL_DATA* p_data,
                                   BOOLEAN check_dch_setup);
...
extern void
bta_hl_build_dch_open_cfm(tBTA_HL* p_evt_data, tBTA_HL_APP_HANDLE app_handle,
                          tBTA_HL_MCL_HANDLE mcl_handle,
                          tBTA_HL_MDL_HANDLE mdl_handle,
                          tBTA_HL_MDEP_ID local_mdep_id, tBTA_HL_MDL_ID mdl_id,
                          tBTA_HL_DCH_MODE dch_mode, BOOLEAN first_reliable,
                          UINT16 mtu, tBTA_HL_STATUS status);
