...
#define BTA_AG_BSRF_FEAT_SPEC                                                  \
    (BTA_AG_FEAT_3WAY | BTA_AG_FEAT_ECNR | BTA_AG_FEAT_VREC |                  \
     ...
     BTA_AG_FEAT_CODEC | BTA_AG_FEAT_VOIP)
...
typedef struct {
    ...
    BOOLEAN retry_with_sco_only; /* indicator to try with SCO only when eSCO
                                    fails */
...
typedef struct {
    tBTM_ESCO_CONN_REQ_EVT_DATA
        conn_data;           /* SCO data for pending conn request */
...
extern void bta_ag_sm_execute(tBTA_AG_SCB* p_scb, UINT16 event,
                              tBTA_AG_DATA* p_data);
...
/* SDP functions */
extern BOOLEAN bta_ag_add_record(UINT16 service_uuid, char* p_service_name,
                                 UINT8 scn, tBTA_AG_FEAT features,
                                 UINT32 sdp_handle);
...
extern BOOLEAN bta_ag_sdp_find_attr(tBTA_AG_SCB* p_scb,
...
extern void bta_ag_at_hfp_cback(tBTA_AG_SCB* p_scb, UINT16 cmd, UINT8 arg_type,
                                char* p_arg, INT16 int_arg);
extern void bta_ag_at_err_cback(tBTA_AG_SCB* p_scb, BOOLEAN unknown,
                                char* p_arg);
