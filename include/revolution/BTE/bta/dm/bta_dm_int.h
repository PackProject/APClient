...
#if (BLE_INCLUDED == TRUE && (defined BTA_GATT_INCLUDED) &&                    \
...
typedef struct {
    ...
    BOOLEAN
    conn_paired_only; /* allow connectable to paired device only or not */
...
typedef struct {
    ...
    UINT32 di_handle[BTA_DI_NUM_MAX]; /* local DI record handle, the first one
                                         is primary record */
} tBTA_DM_DI_CB;
...
typedef struct {
    ...
    BOOLEAN avoid_scatter; /* TRUE to avoid scatternet when av is streaming (be
                              the master) */
