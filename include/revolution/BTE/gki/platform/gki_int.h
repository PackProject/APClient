...
/* The base priority used for pthread based GKI task. below value is to keep it
 * retro compatible. It is recommended to use (GKI_MAX_TASKS+3), this will
 * assign real time priorities GKI_MAX_TASKS- task_id -2 to the thread */
#ifndef GKI_LINUX_BASE_PRIORITY
#define GKI_LINUX_BASE_PRIORITY 30
#endif

/* The base policy used for pthread based GKI task. the sched defines are
 * defined here to avoid undefined values due to missing header file, see
 * pthread functions! Overall it is recommend however to use SCHED_NOMRAL */
...
/* the GKI_timer_update() thread should have the highest real time priority to
 * ensue correct timer expiry.
 */
#ifndef GKI_LINUX_TIMER_TICK_PRIORITY
#define GKI_LINUX_TIMER_TICK_PRIORITY GKI_LINUX_BASE_PRIORITY + 2
#endif

/* the AV timer should preferably run above the gki timer tick to ensure precise
 * AV timing If you observe AV jitter under have load you may increase this one
 */
#ifndef GKI_LINUX_AV_TIMER_PRIORITY
#define GKI_LINUX_AV_TIMER_PRIORITY GKI_LINUX_BASE_PRIORITY + 3
#endif

/* defines by how much the nice value of the PROCESS should be changed. Values
 * allowed: -19 to +19. a negative value give higher priority to btld compared
 * to the default nice value of 20. in case of SCHED_NORMAL, a level of -5
 * should give a good btld/bt performance. In case of real time scheduling,
 * leave default value.
 */
