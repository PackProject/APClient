...
#error                                                                         \
    "An Android.mk file did not include bdroid_CFLAGS and possibly not bdorid_C_INCLUDES"
#endif
...
#define HCI_ACL_DATA_TO_UPPER(p)                                               \
    {                                                                          \
        ((BT_HDR*)p)->event = BT_EVT_TO_BTU_HCI_ACL;                           \
        GKI_send_msg(BTU_TASK, BTU_HCI_RCV_MBOX, p);                           \
    }
#endif

/* Sends SCO data received over HCI to the upper stack. */
#ifndef HCI_SCO_DATA_TO_UPPER
#define HCI_SCO_DATA_TO_UPPER(p)                                               \
    {                                                                          \
        ((BT_HDR*)p)->event = BT_EVT_TO_BTU_HCI_SCO;                           \
        GKI_send_msg(BTU_TASK, BTU_HCI_RCV_MBOX, p);                           \
    }
#endif
...
#define HCI_EVT_TO_UPPER(p)                                                    \
    {                                                                          \
        ((BT_HDR*)p)->event = BT_EVT_TO_BTU_HCI_EVT;                           \
        GKI_send_msg(BTU_TASK, BTU_HCI_RCV_MBOX, p);                           \
    }
#endif
...
#if (!defined(HCI_USE_VARIABLE_SIZE_CMD_BUF) ||                                \
     (HCI_USE_VARIABLE_SIZE_CMD_BUF == FALSE))
...
#define HCI_GET_CMD_BUF(paramlen)                                              \
    ((BT_HDR*)GKI_getbuf(                                                      \
        (UINT16)(BT_HDR_SIZE + HCIC_PREAMBLE_SIZE + (paramlen))))
#endif
...
#define HCI_BLE_ACL_DATA_TO_LOWER(p)                                           \
    bte_main_hci_send((BT_HDR*)(p), (UINT16)(BT_EVT_TO_LM_HCI_ACL |            \
...
#define HCI_SCO_DATA_TO_LOWER(p)                                               \
    bte_main_hci_send((BT_HDR*)(p), BT_EVT_TO_LM_HCI_SCO);
#endif

/* Sends an HCI command received from the upper stack to the BD/EDR HCI
 * transport. */
...
/* Sends an LM Diagnosic command received from the upper stack to the HCI
 * transport. */
#ifndef HCI_LM_DIAG_TO_LOWER
#define HCI_LM_DIAG_TO_LOWER(p)                                                \
    bte_main_hci_send((BT_HDR*)(p), BT_EVT_TO_LM_DIAG);
#endif
...
/* If nonzero, the upper-layer sends at most this number of HCI commands to the
 * lower-layer. */
...
/* Use 2 second for low-resolution systems, override to 1 for high-resolution
 * systems */
 ...
 */
/* if none of them is included then QUICK_TIMER_TICKS_PER_SEC is set to 0 to
 * exclude quick timer */
...
set to FALSE for advanced start-up / shut-down procedures using
USER_HW_ENABLE_API and USER_HW_DISABLE_API macros */
...
/* The default periodic inquiry maximum delay when auto inquiry is enabled,
 * in 1.28 second units. */
#ifndef BTM_DEFAULT_INQ_MAX_DELAY
#define BTM_DEFAULT_INQ_MAX_DELAY 30
#endif

/* The default periodic inquiry minimum delay when auto inquiry is enabled,
 * in 1.28 second units. */
#ifndef BTM_DEFAULT_INQ_MIN_DELAY
#define BTM_DEFAULT_INQ_MIN_DELAY 20
#endif

/* The maximum age of entries in inquiry database in seconds ('0' disables
 * feature). */
#ifndef BTM_INQ_MAX_AGE
#define BTM_INQ_MAX_AGE 0
#endif

/* The maximum age of entries in inquiry database based on inquiry response
 * failure ('0' disables feature). */
...
/* Number of seconds to wait to send an HCI Reset command upon device
 * initialization. */
#ifndef BTM_FIRST_RESET_DELAY
#define BTM_FIRST_RESET_DELAY 0
#endif

/* The number of seconds to wait for controller module to reset after issuing an
 * HCI Reset command. */
 ...
 * Transfer,Bit22 -Telephony) MAJOR_CLASS:0x02 - PHONE MINOR_CLASS:0x0C -
 * SMART_PHONE
 *
 */
#ifndef BTA_DM_COD
...
/* TRUE if default string is used, FALSE if device name is set in the
 * application */
...
/* May be set to the the name of a function used for vendor specific chip
 * initialization */
#ifndef BTM_APP_DEV_INIT
/* #define BTM_APP_DEV_INIT         myInitFunction() */
#endif

/* This is set to TRUE if the busy level change event is desired. (replace ACL
 * change event) */
 ...
 * seconds, a negative response would be sent automatically. It's recommended to
 * use a value between 30 and OBX_TIMEOUT_VALUE 30 is LMP response timeout value
 */
...
**    Lisbon Features
*******************************************/
/* This is set to TRUE if the server Extended Inquiry Response feature is
 * desired. */
...
/* This is set to TRUE if the client Extended Inquiry Response feature is
 * desired. */
 ...
 * Pairing Possible values are BTM_AUTH_AP_YES or BTM_AUTH_AP_NO */
...
/* The L2CAP MPS over Bluetooth; must be in accord with the FCR tx pool size and
 * ACL down buffer size. */
...
/* The percentage of the queue size allowed before a congestion event is sent to
 * the L2CAP client (typically 120%). */
...
/* if application like BTA, Java or script test engine is running on other than
 * BTU thread, */
...
/* Broadcom specific acknowledgement message to ensure fast and robust operation
 * of WUG FIMA procedure. */
...
/* The timeout value (in seconds) for reliable sessions to remain in suspend.
 * 0xFFFFFFFF for no timeout event. */
...
Note:  This pool needs to have enough buffers to hold two times the window size
negotiated in the L2CA_SetFCROptions (2 * tx_win_size)  to allow for
retransmissions. The size of each buffer must be able to hold the maximum MPS
segment size passed in L2CA_SetFCROptions plus BT_HDR (8) + HCI preamble (4) +
...
#define BNEP_SUPPORTS_ALL_UUID_LENGTHS                                         \
    TRUE /* Otherwise it will support only 16bit UUIDs */
#endif
 ...
 * Must be less than the number of buffers in the buffer pool of size
 * AVDT_DATA_POOL_SIZE */
...
/* If the controller can not support sniff mode when the SCO is up, set this to
 * FALSE. */
#ifndef CTP_USE_SNIFF_ON_SCO
#define CTP_USE_SNIFF_ON_SCO FALSE
#endif

/* When ACL link between TL and GW is idle for more than this amount of seconds,
 * the ACL may be put to low power mode. */
...
/* CTP TL would try to re-establish L2CAP channel after channel is disconnected
...
/* When TL does not create another L2CAP channel within this period of time GW
 * declares that it's "Connected Limited". */
#ifndef CTP_TL_CONN_TIMEOUT
#define CTP_TL_CONN_TIMEOUT 5
#endif

/* The delay for ACL to completely disconnect (for intercom) before sending the
 * connect request to GW. */
...
/* The time in seconds to wait for completion of a partial AT command or
...
/* SCO packet type(s) to use (bitmask: see spec), 0 - device default
 * (recommended) */
...
#define HSP2_DEFAULT_VOICE_SETTINGS                                            \
...
/* This is set to the maximum length of path name allowed in the system
 * (_MAX_PATH). */
...
/* This is set to the maximum length of file name allowed in the system
 * (_MAX_FNAME). */
...
#endif
 */
/* Document Format Supported. ASCII comma-delimited list of MIME type:version
string #ifndef BPP_DOC_FORMAT_SUPPORTED #define BPP_DOC_FORMAT_SUPPORTED
"application/vnd.pwg-xhtml-print:1.0,application/vnd.hp-PCL:5E,application/PDF"
#endif
...
#define BPP_CHARACTER_REPERTOIRES
...
#ifndef BPP_1284ID
#define BPP_1284ID                  "\x00\x48MANUFACTURER:ACME
Manufacturing;COMMAND SET:PCL,MPL;MODEL:LaserBeam \?;" #endif
...
#define BPP_MEDIA_TYPES_SUPPORTED
"stationary,continuous-long,photographic-high-gloss,cardstock" #endif
 ...
 * this size of data may be kept in the BPP control block for continuing
 * parsing. The maximum is the size of the biggest GKI buffer (GKI_MAX_BUF_SIZE)
 */
...
/* in case that the received XML object is not complete, the XML parser state
 * machine needs to keep a copy of the data from the last '<' This macro
 * specifies the maximun amount of data for this purpose */
#ifndef BIP_XML_CARRY_OVER_LEN
#define BIP_XML_CARRY_OVER_LEN 100
#endif

/* minimum 4, maximum is 255. The value should be set to the maximum size of
 * encoding string + 1. JPEG2000. If vendor specific format is supported, it
 * might be bigger than 9 */
...
/* Maximum length used to store the 1284 id string (Minimum 2 byte length
 * field). */
...
/* Number of seconds to wait for the application to respond to a protocol
 * request. */
#ifndef HCRP_APPL_RSP_TOUT
#define HCRP_APPL_RSP_TOUT 5
#endif

/* Number of seconds to wait for the peer device to respond to a protocol
 * request. */
#ifndef HCRP_CMD_RSP_TOUT
#define HCRP_CMD_RSP_TOUT 7
#endif

/* Number of seconds between subsequent credit requests to the server when the
 * send watermark has been exceeded. */
...
/* Maximum number of bytes to be reserved for searching for the client's
 * notification record. */
#ifndef HCRP_MAX_NOTIF_DISC_BUF
#define HCRP_MAX_NOTIF_DISC_BUF 300
#endif

/* Maximum number of clients the server will allow to be registered for
 * notifications. */
...
L2CAP_DEFAULT_ERM_POOL_ID is specified to use the HCI ACL data pool.
Note:  This pool needs to have enough buffers to hold two times the window size
negotiated in the tL2CAP_FCR_OPTIONS (2 * tx_win_size)  to allow for
retransmissions. The size of each buffer must be able to hold the maximum MPS
segment size passed in tL2CAP_FCR_OPTIONS plus BT_HDR (8) + HCI preamble (4) +
L2CAP_MIN_OFFSET (11 - as of BT 2.1 + EDR Spec).
*/
...
mode. Range: Minimum 2000 (2 secs) when supporting PBF.
 */
...
mode. Range: Minimum 12000 (12 secs) when supporting PBF.
*/
