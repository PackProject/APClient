...
#define UI_DM_ID 1
#define UI_PRM_ID 20
/* this defines the enabled BTA modules. client may not be defined as those are
 * enabled at run time. they are defined for completeness. please check with
 * bta_sys.h for new modules. BTA_ID_DM serves as flag for BTA_EnableBluetooth()
 * BTA_ID_RES can be used to fs ID in bte_appl.
 */

#define BTAPP_NUM_ID_BLOCKS                                                    \
    ((BTA_ID_MAX / 32) + 1) /* number of 32 bit required to store one bit per  \
                                 btapp id */

#define BTAPP_APPL_MAIL_EVENT(x)                                               \
    (x << 8) /* define bte_appl task mail box event. LSB contains              \
                 BTA_ID_xxx (see bta_sys.h) */
#define BTAPP_APPL_MAIL_EVT                                                    \
    0xff00 /* high byte contains bitmap of application module event */

/* helper macro to copy BTA callack data y into message buffer x, for event data
 * structure z */
#define MEMCPY_APPL_MSG(x, y, z)                                               \
    memcpy((void*)(((UINT8*)x) + sizeof(BT_HDR)), (void*)y, sizeof(z))

/* Event masks for BTE_APPL_TASK */
#define BTE_APPL_STARTUP_EVT                                                   \
    EVENT_MASK(APPL_EVT_0) /* Bluetooth has started                            \
                            */
#define BTE_APPL_SHUTDOWN_EVT                                                  \
    EVENT_MASK(APPL_EVT_1) /* Bluetooth is shutting down */
#define BTE_APPL_SOCKET_RX_EVT                                                 \
    EVENT_MASK(APPL_EVT_2) /* Socket data ready to be read */
#define BTE_APPL_DBUS_RX_EVT                                                   \
    EVENT_MASK(APPL_EVT_3) /* DBUS message ready to be read */
...
typedef struct {
    ...
    /*tBTM_CMPL_CB*/ tBTM_DEV_STATUS_CB*
        p_reset_cplt_cb; /* Current reset_cplt_cb */
    char contacts_db[BTE_APPL_CONTACTS_DB_PATH + 1];
    UINT32 bta_module_state[BTAPP_NUM_ID_BLOCKS]; /* state of enabled bta
                                                     modules */
...
/* global application control block storing global application states and
 * variables */
typedef struct tBTE_APPL_CB_tag {
    sigset_t signal_handler_set; /* signal handler set used by signal handler
                                    thread */
#if (TRUE == BTE_RESET_BAUD_ON_BT_DISABLE)
    tBTAPP_SEMAPHORE shutdown_semaphore; /* used to sync with  terminate handler
                                            initated ops */
#endif
...
extern int BTAPP_enable_bta(const UINT32 bta_module_state[BTAPP_NUM_ID_BLOCKS],
                            int includingFM);
extern int BTAPP_disable_bta(const UINT32 bta_module_state[BTAPP_NUM_ID_BLOCKS],
                             int includingFM);
...
#define BTAPP_SET_BTA_MOD(id, a)                                               \
    { a[id / 32] |= (UINT32)(1 << (id % 32)); }

/* set bit id to 0 (cleared) in UINT32 a[] NO RANGE CHECK */
#define BTAPP_CLEAR_BTA_MOD(id, a)                                             \
    { a[id / 32] &= (UINT32) !(1 << (id % 32)); }

/* tests if bit id is 1 in UINT32 a[] NO RANGE CHECK */
#define BTAPP_BTA_MOD_IS_SET(id, a) (a[id / 32] & (UINT32)(1 << (id % 32)))

/* update this list either via btld.txt or directly here by adding the new
 * profiles as per bta_sys.h. each xxx_LISTx may only contain 32 bits */
#ifndef BTAPP_BTA_MODULES_LIST0
#define BTAPP_BTA_MODULES_LIST0                                                \
    ((1 << BTA_ID_DM) | (1 << BTA_ID_DG) | (1 << BTA_ID_AG) |                  \
     (1 << BTA_ID_OPC) | (1 << BTA_ID_OPS) | (1 << BTA_ID_FTS) |               \
     (1 << BTA_ID_PAN) | (1 << BTA_ID_PR) | (1 << BTA_ID_SC) |                 \
     (1 << BTA_ID_AV) | (1 << BTA_ID_HH) | (1 << BTA_ID_PBS) |                 \
     (1 << BTA_ID_FMTX) | (1 << BTA_ID_JV) | (1 << BTA_ID_MSE))
#endif
...
#define BTAPP_BTA_MODULES_LIST1                                                \
    ((1 << (BTA_ID_MAX - BTAPP_LIST1_BLOCK)) |                                 \
     (1 << (BTA_ID_MSE - BTAPP_LIST1_BLOCK)) | 0)
#endif
...
#define BTAPP_DEFAULT_MODULES                                                  \
    {BTAPP_BTA_MODULES_LIST0} /* max 32 modules IDs                            \
                               */
#elif (2 == BTAPP_NUM_ID_BLOCKS)
#define BTAPP_DEFAULT_MODULES                                                  \
    {BTAPP_BTA_MODULES_LIST0, BTAPP_BTA_MODULES_LIST1} /* 64 module IDs max */
#else
