...
/* by default on shutdown, baudrate is reset 115kbits. this should NOT be need
 * for platforms that kill BTE driver and remove/reset BT chip
 */
...
enum {
    ...
    /* -------------------------------------------- */
    BTE_MODE_SERIAL_APP, /* OUT  OUT             OFF     OFF     OFF     Sample
                            serial port application      */
    BTE_MODE_APPL, /* IN   OUT             OFF     OFF     ON      Target used
                      with Tester through RPC */
    BTE_MODE_RESERVED, /* OUT  IN              OFF     ON      OFF     Reserved
                        */
    BTE_MODE_SAMPLE_APPS, /* IN   IN              OFF     ON      ON      Sample
                             applications (ICP/HSP)       */
    BTE_MODE_DONGLE, /* not yet supported    ON      OFF     OFF     Dongle mode
                      */
    BTE_MODE_APPL_PROTOCOL_TRACE, /* this is a fake mode do allow protocol
                                     tracing in application without rpc */
    BTE_MODE_INVALID
};
...
extern void bte_hcisu_userial_oper(tUSERIAL_MSG_CBACK* p_cback, UINT8 port,
                                   UINT8 op, UINT8 option);
